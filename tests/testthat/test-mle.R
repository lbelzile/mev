...
test_that("GEV: `fpar` input for `fit.gev`", {
...
test_that("GEV: profile yields same maximum as full likelihood", {
...
test_that("GP: `fpar` input for `fit.gpd`", {
...
mle <- fit.pp(xdat, np = 10)
test_that("IPP: profile yields same maximum as full likelihood", {
  expect_equal(fit.pp(xdat,
                           np = 10,
...
pp.score(par = mle$param, dat  = xdat, u = 0, np = 10)
