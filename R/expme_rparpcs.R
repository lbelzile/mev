...
rparpcs <- function(n, Lambda = NULL, Sigma = NULL, df = NULL,
  ...
  for(i in 1:length(tabu)){
    ...
    if(model == "xstud"){
      ...
      if(model == "xstud"){
        ang[-j, (accu+1L):(accu+tabu[i])] <-
          pmax(TruncatedNormal::mvrandt(n = tabu[i], u = rep(Inf, D - 1), l = rep(1, D - 1), df = df + 1,
                                        Sig = (Sigma[-j, -j] - Sigma[-j, j, drop = FALSE] %*% Sigma[j, -j, drop = FALSE]) / (df + 1)), 0)^df
      } else if(model == "br"){
        if(!is.null(Lambda)){
          ang[-j, (accu+1L):(accu+tabu[i])] <- exp(TruncatedNormal::mvrandn(n = tabu[i], u = rep(Inf, D - 1),
                                                    Sig = 2 * (outer(Lambda[-j,j], Lambda[j,-j], FUN = "+") - Lambda[-j,-j]),
                                                    l = 2 * Lambda[j,-j]) - 2*Lambda[j,-j])
        } else if (!is.null(Sigma)){
          me <- diag(Sigma)[-j]/2 + Sigma[j,j]/2 - Sigma[j,-j]
          Ti <- T1; Ti[, c(1L, j)] <- T1[, c(j, 1L)]
          ang[-j, (accu+1L):(accu+tabu[i])] <- exp(TruncatedNormal::mvrandn(n = tabu[i], u = rep(Inf, D - 1),
                                                                            l = -me, Sig = Ti %*% Sigma %*% t(Ti)) + me)
        }
      }
...
weightsBR <- function(z, Lambda, method = c("mvtnorm", "mvPot", "TruncatedNormal"), riskf = c("max", "min")){
  ...
  if(method == "mvtnorm"){
    for(j in 1:D){
      weights[j] <- mvtnorm::pmvnorm(lower = rep(-Inf, D - 1), upper = si*(2*Lambda[-j,j] + log(z[-j]) - log(z[j])),
    ...
    for(j in 1:D){
      weights[j] <- mvPot::mvtNormQuasiMonteCarlo(p = genVec$primeP,
                                                  upperBound = si*(2*Lambda[-j,j] + log(z[-j]) - log(z[j])),
    ...
    for(j in 1:D){
      weights[j] <- TruncatedNormal::mvNqmc(l = rep(-Inf, D - 1), n = 1e5,
                                            u = si*(2*Lambda[-j,j] + log(z[-j]) - log(z[j])),
  ...
  for(j in 1:D){
    ...
    if(method == "mvtnorm"){
      weights[j] <- mvtnorm::pmvnorm(lower = rep(-Inf, D - 1),
                                     upper = si*(log(z[-j]/z[j]) + diag(Sigma)[-j]/2 + Sigma[j,j]/2 - Sigma[j,-j]),
                                     sigma =  Ti %*% Sigma %*% t(Ti))
    } else if (method == "mvPot"){
      weights[j] <- mvPot::mvtNormQuasiMonteCarlo(p = genVec$primeP,
                                                  upperBound = si*(log(z[-j]/z[j]) + diag(Sigma)[-j]/2 + Sigma[j,j]/2 - Sigma[j,-j]),
                                                  cov = Ti %*% Sigma %*% t(Ti),
                                                  genVec = genVec$genVec)[1]
    } else if(method == "TruncatedNormal"){
      weights[j] <- TruncatedNormal::mvNqmc(l = rep(-Inf, D - 1), n = 1e5,
                                            u = si*(log(z[-j]/z[j]) + diag(Sigma)[-j]/2 + Sigma[j,j]/2 - Sigma[j,-j]),
  ...
  if(method == "mvtnorm"){
    for(j in 1:D){
      weights[j] <- mvtnorm::pmvt(lower = rep(-Inf, D - 1), df = df + 1,
                                  upper = si*(exp((log(z[-j]) - log(z[j]))/df) - Sigma[-j,j]),
    ...
    for(j in 1:D){
      weights[j] <- mvPot::mvTProbQuasiMonteCarlo(p = genVec$primeP,
                                                  upperBound = si*(exp((log(z[-j]) - log(z[j]))/df) - Sigma[-j,j]),
    ...
    for(j in 1:D){
      weights[j] <- TruncatedNormal::mvTqmc(l = rep(-Inf, D - 1), df = df + 1, n = 1e5,
                                            u = si*(exp((log(z[-j]) - log(z[j]))/df) - Sigma[-j,j]),
