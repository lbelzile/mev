...
fit.extgp <- function(
  ...
  if ("pwm" %in% method) {
    if (!requireNamespace("gmm", quietly = TRUE)) {
      stop(
        "Package \"gmm\" needed for this function to work. Please install it.",
        call. = FALSE
      )
    }
  }
  if (confint) {
    if (!requireNamespace("boot", quietly = TRUE)) {
      stop(
        "Package \"boot\" needed for this function to work. Please install it.",
        call. = FALSE
      )
    }
  }
  ...
  if (any(method == "pwm")) {
    if (model == 0) {
      ...
      if (confint) {
        fit.pwm.boot <- boot::boot(
          data = data,
          statistic = extgp.fit.pwm.boot,
          R = R,
          type = 0,
          sigma0 = init[1],
          ncpus = ncpus,
        ...
        CI.pwm.sigma <- boot::boot.ci(
          boot.out = fit.pwm.boot,
          index = 1,
          type = "perc"
        )$perc[4:5]
        CI.pwm.xi <- boot::boot.ci(
          boot.out = fit.pwm.boot,
          index = 2,
          type = "perc"
        )$perc[4:5]
      ...
      if (plots) {
        qextgp.pwm <- qextgp(
          p = c(1:length(data)) / (length(data) + 1),
          type = 0,
          sigma = fit.pwm[1],
          xi = fit.pwm[2]
        )
        ...
        if (confint) {
          q.pwm.boot <- mapply(
            FUN = qextgp,
            p = list(c(1:length(data)) / (length(data) + 1)),
            type = list(1),
            sigma = as.list(fit.pwm.boot$t[, 1]),
            xi = as.list(fit.pwm.boot$t[, 2])
          )
      ...
      if (plots) {
        ...
        if (confint) {
          q.pwm.boot <- mapply(
            ...
            sigma = as.list(fit.pwm.boot$t[, 2]),
            xi = as.list(fit.pwm.boot$t[, 3])
          )
          q.pwm.L <- apply(q.pwm.boot, 1, quantile, 0.025, na.rm = TRUE)
          q.pwm.U <- apply(q.pwm.boot, 1, quantile, 0.975, na.rm = TRUE)
        }
      ...
      if (plots) {
        ...
        if (confint) {
          q.pwm.boot <- mapply(
            ...
            delta = as.list(fit.pwm.boot$t[, 2]),
            sigma = as.list(fit.pwm.boot$t[, 3]),
            xi = as.list(fit.pwm.boot$t[,
              4
            ])
          )
          q.pwm.L <- apply(q.pwm.boot, 1, quantile, 0.025, na.rm = TRUE)
      ...
      if (plots) {
        ...
        if (confint) {
          q.pwm.boot <- mapply(
            ...
            p = list(c(1:length(data)) / (length(data) + 1)),
            type = list(4),
            prob = as.list(fit.pwm.boot$t[,
              1
            ]),
            kappa = as.list(fit.pwm.boot$t[, 2]),
            delta = as.list(fit.pwm.boot$t[, 3]),
            sigma = as.list(fit.pwm.boot$t[,
              4
            ]),
            xi = as.list(fit.pwm.boot$t[, 5])
          )
          q.pwm.L <- apply(q.pwm.boot, 1, quantile, 0.025, na.rm = TRUE)
          q.pwm.U <- apply(q.pwm.boot, 1, quantile, 0.975, na.rm = TRUE)
        }
  ...
  if (any(method == "mle")) {
    if (model == 0) {
      ...
      if (confint) {
        fit.mle.boot <- boot::boot(
          data = data,
          statistic = extgp.fit.ml.boot,
          R = R,
          type = 0,
          sigma0 = init[1],
          xi0 = init[2],
        ...
        CI.mle.sigma <- boot::boot.ci(
          boot.out = fit.mle.boot,
          index = 1,
          type = "perc"
        )$perc[4:5]
        CI.mle.xi <- boot::boot.ci(
          boot.out = fit.mle.boot,
          index = 2,
          type = "perc"
        )$perc[4:5]
      ...
      if (plots) {
        ...
        if (confint) {
          q.mle.boot <- mapply(
            ...
            p = list(c(1:length(data)) / (length(data) + 1)),
            type = list(1),
            sigma = as.list(fit.mle.boot$t[, 1]),
            xi = as.list(fit.mle.boot$t[, 2])
          )
      ...
      if (plots) {
        ...
        if (confint) {
          q.mle.boot <- mapply(
            ...
            p = list(c(1:length(data)) / (length(data) + 1)),
            type = list(1),
            kappa = as.list(fit.mle.boot$t[,
              1
            ]),
            sigma = as.list(fit.mle.boot$t[, 2]),
            xi = as.list(fit.mle.boot$t[, 3])
          )
          q.mle.L <- apply(q.mle.boot, 1, quantile, 0.025, na.rm = TRUE)
          q.mle.U <- apply(q.mle.boot, 1, quantile, 0.975, na.rm = TRUE)
        }
      ...
      if (plots) {
        ...
        if (confint) {
          q.mle.boot <- mapply(
            ...
            delta = as.list(fit.mle.boot$t[, 2]),
            sigma = as.list(fit.mle.boot$t[, 3]),
            xi = as.list(fit.mle.boot$t[,
              4
            ])
          )
          q.mle.L <- apply(q.mle.boot, 1, quantile, 0.025, na.rm = TRUE)
      ...
      if (plots) {
        ...
        if (confint) {
          q.mle.boot <- mapply(
            ...
            p = list(c(1:length(data)) / (length(data) + 1)),
            type = list(4),
            prob = as.list(fit.mle.boot$t[,
              1
            ]),
            kappa = as.list(fit.mle.boot$t[, 2]),
            delta = as.list(fit.mle.boot$t[, 3]),
            sigma = as.list(fit.mle.boot$t[,
              4
            ]),
            xi = as.list(fit.mle.boot$t[, 5])
          )
          q.mle.L <- apply(q.mle.boot, 1, quantile, 0.025, na.rm = TRUE)
          q.mle.U <- apply(q.mle.boot, 1, quantile, 0.975, na.rm = TRUE)
        }
  ...
  if (plots) {
    ...
    hist(
      data,
      breaks = c(0:30) * max(data + 1e-1) / 30,
      freq = FALSE,
      xlim = range(x),
      xlab = "Rainfall amounts [mm]",
      ylab = "Density",
      main = "",
      col = "lightgrey"
    )
    ...
    if (confint) {
      if (any(method == "pwm")) {
        polygon(
          x = c(sort(data), sort(data, decreasing = TRUE)),
    ...
    if (any(method == "pwm")) {
      lines(sort(data), qextgp.pwm, lty = 2, type = "b", pch = 20, col = 2)
    }
    if (any(method == "mle")) {
      lines(sort(data), qextgp.mle, lty = 1, type = "b", pch = 20, col = 4)
    }
...
pextgp.G <- function(u, type = 1, prob, kappa, delta) {
  ...
  if (type %in% c(1, 3, 4)) {
    if (missing(kappa)) {
      ...
      if (!isTRUE(all(is.numeric(kappa), length(kappa) == 1L, kappa > 0))) {
  ...
  if (type %in% c(2, 3, 4)) {
    if (missing(delta)) {
      ...
      if (!isTRUE(all(is.numeric(delta), length(delta) == 1L, delta > 0))) {
  ...
  if (type == 4) {
    if (missing(prob)) {
      ...
      if (
        !isTRUE(all(is.numeric(prob), length(prob) == 1L, prob >= 0, prob <= 1))
      ) {
...
dextgp.G <- function(
  ...
  if (log == FALSE) {
    if (type == 0) {
      ...
      return(
        (kappa / 2) *
          (1 - pbeta((1 - u)^delta, 1 / delta, 2))^(kappa / 2 - 1) *
          dbeta((1 - u)^delta, 1 / delta, 2) *
          delta *
          (1 -
            u)^(delta - 1)
      )
    ...
    if (type == 0) {
      ...
      return(
        dbeta((1 - u)^delta, 1 / delta, 2, log = TRUE) +
          log(delta) +
          (delta - 1) * log(1 - u)
      )
    } else if (type == 3) {
      return(
        log(kappa / 2) +
          (kappa / 2 - 1) * log(1 - pbeta((1 - u)^delta, 1 / delta, 2)) +
          dbeta((1 - u)^delta, 1 / delta, 2, log = TRUE) +
          log(delta) +
          (delta - 1) * log(1 - u)
      )
      ...
      return(log(
        prob * kappa * u^(kappa - 1) + (1 - prob) * delta * u^(delta - 1)
      ))
    }
  }
}
...
qextgp.G <- function(u, type = 1, prob = NA, kappa = NA, delta = NA) {
  ...
  if (type == 0) {
    ...
    dummy.func <- function(u, p, prob = NA, kappa = NA, delta = NA) {
      return(
        pextgp.G(u = u, prob = prob, kappa = kappa, delta = delta, type = 4) - p
      )
    }
...
rextgp.G <- function(
  ...
  if (type != 4 | (type == 4 & direct)) {
    ...
    if (censoring[1] == 0 & censoring[2] == 1) {
      ...
      res <- c()
      res[components == 1] <- qextgp.G(
        unifsamp[components == 1],
        prob = NA,
        kappa = kappa,
        delta = NA,
        type = 1
      )
      res[components == 2] <- qextgp.G(
        unifsamp[components == 2],
        prob = NA,
        kappa = delta,
        delta = NA,
        type = 1
      ...
      components <- sample(
        ...
        size = n,
        replace = TRUE,
        prob = c(new.prob, 1 - new.prob)
      )
      res <- c()
      res[components == 1] <- qextgp.G(
        x.L + (x.U - x.L) * unifsamp[components == 1],
        prob = NA,
        kappa = kappa,
        delta = NA,
        type = 1
      ...
      res[components == 2] <- qextgp.G(
        x.L + (x.U - x.L) * unifsamp[components == 2],
        prob = NA,
        kappa = delta,
        delta = NA,
        type = 1
...
pextgp <- function(
  ...
  return(pextgp.G(
    mev::pgp(q, scale = sigma, shape = xi),
    prob = prob,
    kappa = kappa,
    delta = delta,
    type = type
...
dextgp <- function(
  ...
  if (log == FALSE) {
    return(
      dextgp.G(
        mev::pgp(x, scale = sigma, shape = xi),
        prob = prob,
        kappa = kappa,
        delta = delta,
        type = type
      ) *
        mev::dgp(x, scale = sigma, shape = xi)
    )
  } else {
    return(
      dextgp.G(
        ...
        mev::dgp(x, scale = sigma, shape = xi, log = TRUE)
    )
  }
}
...
qextgp <- function(
  ...
  if (isTRUE(all.equal(step, 0, check.attributes = FALSE))) {
    return(mev::qgp(
      qextgp.G(p, prob = prob, kappa = kappa, delta = delta, type = type),
      scale = sigma,
      shape = xi
    ))
  } else {
...
extgp.pwm <- function(
  ...
  H.L <- ifelse(
    censoring[1] > 0,
    mev::pgp(censoring[1], loc = 0, scale = sigma, shape = xi),
    0
  )
  H.U <- ifelse(
    censoring[2] < Inf,
    mev::pgp(censoring[2], loc = 0, scale = sigma, shape = xi),
    1
  )
  ...
  if (!empiric) {
    E2 <- ((1 - F.L)^(orders + 1) - (1 - F.U)^(orders + 1)) /
      (prob.LU * (orders + 1))
    ...
    if (type == 0) {
      ...
      for (i in 1:length(orders)) {
        E1[i] <- (kappa / prob.LU) *
          sum(
            (-1)^c(0:orders[i]) *
              choose(orders[i], c(0:orders[i])) *
              beta.coefs[
                1:(orders[i] +
                  1)
              ] *
              probs.beta[1:(orders[i] + 1)]
          )
      }
      return((sigma / xi) * (E1 - E2))
    } else if (type == 2) {
      ...
      for (i in 1:length(orders)) {
        ...
        numer.j <- expos1 *
          ((1 - H.U)^expos2 - (1 - H.L)^expos2) +
          expos2 * ((1 - H.L)^expos1 - (1 - H.U)^expos1)
        denom.j <- expos1 * expos2

        E1[i] <- (1 / (delta^(orders[i] + 1) * prob.LU)) *
          sum(
            (-1)^c(0:orders[i]) *
              choose(orders[i], c(0:orders[i])) *
              (1 +
                delta)^(orders[i] - c(0:orders[i]) + 1) *
              numer.j /
              denom.j
          )
      }
      ...
      for (i in 1:length(orders)) {
        ...
        const <- choose(orders[i], l.vals) *
          choose(l.vals, m.vals) *
          (-1)^l.vals *
          prob^m.vals *
          (1 - prob)^(l.vals - m.vals)
        beta.coefs1 <- beta(
          kappa * (m.vals + 1) + delta * (l.vals - m.vals),
          1 - xi
        )
        beta.coefs2 <- beta(
          kappa * m.vals + delta * (l.vals - m.vals + 1),
          1 - xi
        )
        probs.beta1 <- pbeta(
          H.U,
          shape1 = kappa * (m.vals + 1) + delta * (l.vals - m.vals),
          shape2 = 1 - xi
        ) -
          pbeta(
            H.L,
        ...
        probs.beta2 <- pbeta(
          H.U,
          shape1 = kappa * m.vals + delta * (l.vals - m.vals + 1),
          shape2 = 1 - xi
        ) -
          pbeta(
            H.L,
          ...
          sum(
            const *
              (prob *
                ...
                beta.coefs1 *
                probs.beta1 +
                (1 - prob) * delta * beta.coefs2 * probs.beta2),
            na.rm = TRUE
          )
      }
      return((sigma / xi) * (E1 - E2))
    ...
    for (i in 1:length(orders)) {
      res[i] <- mean(
        X * (1 - (F.L + (F.U - F.L) * unifsamp))^orders[i],
        na.rm = TRUE
      )
    }
    return(res)
...
extgp.fit.pwm <- function(
  ...
  if (type == 0) {
    ...
    res <- gmm::gmm(
      ...
      theta0,
      optfct = "nlminb",
      lower = c(1e-04, 1e-04, 1e-04, 1e-6),
    ...
    if (is.na(kappa0)) {
      kappa0 <- mean(x[x < quantile(x, 0.1)]) /
        (quantile(x, 0.1) - mean(x[x < quantile(x, 0.1)]))
    }
    if (is.na(delta0)) {
      delta0 <- kappa0 + 0.01
    }
    ...
    res <- gmm::gmm(
      ...
      theta0,
      optfct = "nlminb",
      lower = c(0, 1e-04, 0, 1e-04, 1e-6),
      upper = c(1, Inf, Inf, Inf, 0.99),
      vcov = "iid"
    )
    thetahat <- res$coefficients
...
extgp.nll <- function(theta, x, censoring, rounded, type) {
  ...
  if (type == 0) {
    if (theta[1] <= 0 | theta[2] <= 1e-6 | theta[2] > 0.99) {
      ...
      censor1 <- ifelse(
        censoring[1] > 0,
        pextgp(censoring[1], sigma = theta[1], xi = theta[2], type = 0),
        0
      )
      censor2 <- ifelse(
        censoring[2] < Inf,
        pextgp(censoring[2], sigma = theta[1], xi = theta[2], type = 0),
        1
      )
      contrib.cens1 <- ifelse(
        length(x.cens1) > 0,
        length(x.cens1) * log(censor1),
        0
      )
      contrib.cens2 <- ifelse(
        length(x.cens2) > 0,
        length(x.cens2) * log(1 - censor2),
        0
      )
      contrib.not.cens <- ifelse(
        rounded == 0,
        sum(
          dextgp(
            x.not.cens,
        ...
        sum(log(
          pextgp(
            ...
            pextgp(x.not.cens, sigma = theta[1], xi = theta[2], type = 0)
        ))
      )
