...
#'
#' These functions estimate the extremal index using an approximate
#'  sample from the Frechet distribution.
#'
#' The \bold{Smith} estimator: suppose \eqn{Z(x)} is simple max-stable vector (i.e., with unit Frechet marginals). Then
#'  \eqn{1/Z} is unit exponential and \eqn{1/\max(Z(s_1), Z(s_2))}
#'  is exponential with rate \eqn{\theta = \max\{Z(s_1), Z(s_2)\}}.
...
#' is restricted to the interval \eqn{[1,3]}.
...
#' the Smith estimator if we do not censor the data (\code{p = 0})
...
#' If there are no missing observations, the routine \code{fmadogram}
#' from the \code{SpatialExtremes} package should be preferred as it is
#' noticeably faster.
#'
...
#' to the unit Frechet scale, either parametrically or nonparametrically unless \code{margtrans = "none"}.
#' If \code{margtrans = "gev"}, a parametric GEV model is fitted to each column of \code{dat} using maximum likelihood
#'  estimation and transformed back using the probability integral transform. If \code{method = "emp"},
...
#' @param xdat an \code{n} by \code{D} matrix of unit Frechet observations
#' @param coord an optional \code{d} by \code{D} matrix of location coordinates
#' @param estimator string indicating which estimator to compute, one of \code{smith}, \code{schlather} or \code{fmado}.
#' @param margtrans string indicating which method to use to transform the margins to unit Frechet scale, either \code{emp} for nonparametric transformation via rank transform, \code{gev} for fit of generalized extreme value distribution to marginals, or \code{none}
...
#' @param plot logical; should cloud or matrix of pairwise empirical estimates be plotted? Default to \code{TRUE}.
...
xdep.xcoef <- function(
  ...
  if (!is.null(args$method) & !is.null(args$standardize)) {
    standardize <- args$standardize
    method <- args$method
  } else {
    if (margtrans %in% c("gev", "emp")) {
      standardize <- TRUE
      method <- switch(margtrans, "gev" = "parametric", "emp" = "nonparametric")
  ...
  dat <- xdat
  if (is.null(coord) && !is.null(args$loc)) {
    coord <- args$loc
  }
  if (!is.null(coord)) {
    stopifnot(is.matrix(dat), ncol(dat) >= 2, nrow(coord) == ncol(dat))
  }
  ...
  if (standardize && estimator != "fmado") {
    if (method == "nonparametric") {
      for (j in 1:ncol(dat)) {
        ...
        fr[, j] <- -1 /
          log(rank(dat[, j], na.last = "keep", ties.method = "random") / nj)
      }
    } else {
      #parametric
      for (j in 1:ncol(dat)) {
        fr[, j] <- -1 /
          log(
            do.call(
              what = mev::pgev,
              c(list(q = dat[, j]), fit.gev(dat[, j])$estimate)
            )
          )
      }
    }
  ...
  if (estimator == "schlather") {
    ...
    transfo_fr <- sweep(x = fr, MARGIN = 2, STATS = harmo_mean, FUN = "/")
  }
  ...
  if (!is.null(coord)) {
    ...
    if (estimator == "schlather") {
      ...
      ubin <- unique(bin)
      binned_theta <- as.vector(by(
        data = cbind(theta_vals, overlap_size),
        INDICES = bin,
        FUN = function(xmat) {
      ...
      reslist$binned <- na.omit(cbind(
        h = h[ubin],
        binned.extcoef = binned_theta
      ))
    }
...
plot.mev_extcoef <- function(x, ...) {
  ...
  if (!is.null(x$dist)) {
    ymax <- 1.1 *
      min(
        c(max(c(2.1, x$extcoef), na.rm = TRUE), 3),
        na.rm = TRUE
      )
    ...
    d <- max(x$ind)
    rmat <- diag(d)
    rmat[upper.tri(rmat, diag = FALSE)] <- x$extcoef
    rmat[lower.tri(rmat)] <- 1 # t(rmat)[lower.tri(rmat)]
    image(
      x = 1:d,
      y = 1:d,
      z = rmat,
      ...
      col = gray.colors(100, start = 0, end = 1, rev = TRUE),
      xlab = "",
      ylab = "",
      main = ""
    )
    legend(
      ...
      fill = gray.colors(5, start = 0, end = 1, rev = TRUE),
      bty = "n",
      legend = seq(1, 2, length.out = 5)
    )
    title(
      main = "Extremal coefficient",
      sub = switch(
...
extcoef <- function(
  ...
  # .Deprecated(new = "xdep.xcoef", package = "mev")
  # Backward compatibility
  args <- list(...)
  if (missing(xdat) & !is.null(args$dat)) {
    xdat <- args$dat
