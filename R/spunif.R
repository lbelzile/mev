...
spunif <- function(x, thresh, scale = NULL, shape = NULL) {
  ...
  spunif_univ <- function(x, thresh, scale = NULL, shape = NULL) {
    ...
    un <- x[!isna]
    below <- (un < thresh)
    n <- sum(!isna) # changed June 1st 2025, previously length(x)
    zeta <- 1 - sum(below) / n #proportion above
    if (zeta == 0) {
      stop("Threshold is too high, no data above")
    }
    un[below] <- rank(un, ties.method = "random", na.last = "keep")[below] /
      (n + 1)
    un[!below] <- 1 -
      zeta * (1 + shape * (un[!below] - thresh) / scale)^(-1 / shape)
  ...
  if (is.vector(x)) {
    ...
    if (length(thresh) != ncol(x)) {
      stop(
        "Invalid input: \"thresh\" must be of the same length as \"ncol(x)\""
      )
    }
