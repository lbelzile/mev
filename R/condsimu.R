...
#' @param n sample size. Default to 1.
#' @return an n by d matrix of conditional simulations
rcondmvtnorm <- function(n = 1, ind, x, lbound = rep(-Inf, length(ind)), ubound = rep(Inf, length(ind)),
  ...
  if(all(c(isTRUE(all.equal(lbound, rep(-Inf, length(ind)))), isTRUE(all.equal(ubound, rep(Inf, length(ind))))))){
    return(1)
  }
  if(length(x) == 0){ #Unconditional simulation
    switch(model,
             norm = TruncatedNormal::mvNcdf(n = n, l = lbound - mu, u = ubound - mu, Sig = Sigma),
             stud = TruncatedNormal::mvTcdf(n = n, l = lbound - mu, u = ubound - mu, Sig = Sigma, df = df)
      )
  } else {
