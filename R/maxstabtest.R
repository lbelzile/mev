...
#' @param xdat matrix or array of max-stable observations, typically block maxima. The first dimension should consist of replicates
...
test.maxstab <- function(
  ...
  if (missing(xdat)) {
    ...
    # Aliases, for compatibility reason
    dat <- xdat
  }
  if (is.null(dim(dat))) {
  ...
  if (!d %in% 2:3) {
    stop(
      "\"dat\" must be a n by D matrix or else an (n by D by D) array if observations are gridded"
    )
  }
  D <- prod(dim(dat)[2:d])
  n <- dim(dat)[1]
  ...
  if (B < 20) {
    stop(
      "Not enough bootstrap replications to compute 95% confidence intervals."
    )
  }
  if (m < 1 && m > D) {
  ...
  gdat <- matrix(
    apply(dat, 2:d, function(x) {
      -log(
        -log(
          rank(x, na.last = NA, ties.method = ties.method) / (n + 1) # + runif(length(x), min = -0.5/n, max = 0.5/n)
        )
      )
    }),
    nrow = n,
  ...
  if ((ntuples * n) > 10000L) {
    warning(
      "The number of statistics is higher than 10 000: consider reducing \"nmax\"."
    )
  }
  ...
  for (i in 1:ntuples) {
    ...
    csamp[(n * (i - 1) + 1):(n * i)] <- csamp[(n * (i - 1) + 1):(n * i)] -
      min(
        max(log(n) - log(sum(exp(-csamp[(n * (i - 1) + 1):(n * i)]))), 0),
        log(m)
      )
  }
  if (ntuples * n < 1000) {
    ...
    bsamp[1, ] <- quantile(
      mev::pgev(csamp, loc = 0, scale = 1, shape = 0),
      p,
      type = 3
    )
  }
  ...
  for (b in 2:B) {
    ...
    for (i in 1:ntuples) {
      csamp[(n * (i - 1) + 1):(n * i)] <- apply(
        gdat[sample.int(n, n, replace = TRUE), indm[, i]],
        1,
        max
      )
      csamp[(n * (i - 1) + 1):(n * i)] <- csamp[(n * (i - 1) + 1):(n * i)] -
  ...
  res <- list(p = p, estimate = estimate)
  class(res) <- "mev_test_maxstab"
  if (isTRUE(plot)) {
    plot(res)
  }
...
#' @export
plot.mev_test_maxstab <- function(x, ...) {
  matplot(
    x$p,
    x$estimate - x$p,
    type = "l",
    lty = c(1, 2, 2),
    col = c(1, "grey80", "grey80"),
    ylim = c(
      max(-1, 1.2 * min(x$estimate - x$p)),
      min(1.2 * max(x$estimate - x$p), 1)
    ),
    panel.first = {
      abline(h = 0)
    },
...
maxstabtest <- function(
  ...
  .Deprecated(new = "test.maxstab", package = "mev")
  test.maxstab(
    xdat = dat,
    m = m,
    nmax = nmax,
