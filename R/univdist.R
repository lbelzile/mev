...
gpd.ll <- function(par, dat, tol = 1e-05) {
  ...
  if (abs(xi) > tol) {
    if (xi > -1) {
      #Most scenarios
      as.vector(
        -n * log(sigma) - (1 + 1 / xi) * sum(log(pmax(1 + xi / sigma * dat, 0)))
      )
...
gpd.ll.optim <- function(par, dat, tol = 1e-05) {
  ...
  if (abs(xi) > tol) {
    if (!isTRUE(all.equal(xi, -1))) {
      length(dat) *
        log(sigma) +
        (1 + 1 / xi) * sum(log(pmax(1 + xi / sigma * dat, 0)))
    } else {
...
gpd.score <- function(par, dat) {
  ...
  if (!isTRUE(all.equal(0, xi, check.attributes = FALSE))) {
    c(
      sum(dat * (xi + 1) / (sigma^2 * (dat * xi / sigma + 1)) - 1 / sigma),
      sum(
        -dat *
          (1 / xi + 1) /
          (sigma *
    ...
    c(
      sum((dat - sigma) / sigma^2),
      sum(1 / 2 * (dat - 2 * sigma) * dat / sigma^2)
    )
  }
}
...
gpd.infomat <- function(
  ...
  if (method == "obs") {
    ...
    if (any((1 + xi * dat / sigma) < 0)) {
      stop(
        "Data outside of range specified by parameter, yielding a zero likelihood"
      )
    }
    c11 <- (length(dat) -
      (1 + xi) * sum(dat * (2 * sigma + xi * dat) / (sigma + xi * dat)^2)) /
      (sigma^2)
    if (!isTRUE(all.equal(xi, 0, check.attributes = FALSE))) {
      c12 <- (1 + xi) *
        sum(dat / (sigma + xi * dat)^2) /
        xi -
        sum(dat / (sigma + xi * dat)) /
          (sigma *
            xi)
      c22 <- (2 / xi^2) *
        sum(dat / (sigma + xi * dat)) -
        2 * sum(log(pmax(dat * xi / sigma + 1, 0))) / (xi^3) +
...
#' @export
#' @keywords internal
#' @return a vector of length containing the derivative of the
gev.score <- function(par, dat) {
  ...
  if (!isTRUE(all.equal(xi, 0, tolerance = 1e-10, check.attributes = FALSE))) {
    c(
      sum(
        -(pmax(0, -(mu - dat) * xi / sigma + 1))^(-1 / xi - 1) /
          sigma -
          xi *
            (1 / xi + 1) /
            (sigma *
      ...
      sum(
        -(dat - mu) *
          ...
          sigma^2 +
          (dat - mu) * (xi + 1) / (sigma^2 * ((dat - mu) * xi / sigma + 1)) -
          1 / sigma
      ),
      sum(
        -(mu - dat) *
          ...
          (log1p(-(mu - dat) * xi / sigma) /
            xi^2 -
            (mu - dat) / (sigma * ((mu - dat) * xi / sigma - 1) * xi)) /
    ...
    c(
      sum(-exp(mu / sigma - dat / sigma) / sigma + 1 / sigma),
      sum(
        mu *
...
#'
#' @return a 3 by 3 matrix containing the expected or observed information
#' @inheritParams gev
#' @param nobs number of observations
#' @export
#' @keywords internal
gev.infomat <- function(
  par,
  dat,
  method = c("obs", "exp"),
  nobs = length(dat)
  ...
  if (method == "exp") {
      ...
      return(
        nobs *
          cbind(
            c(
              1 / sigma^2,
              -0.422784335098467 / sigma^2,
              0.41184033042644 / sigma
            ),
            c(
              -0.422784335098467 / sigma^2,
              1.82368066085288 / sigma^2,
              0.332484907160274 / sigma
            ),
            c(
              0.41184033042644 / sigma,
              0.332484907160274 / sigma,
    ...
    infomat <- nobs *
      cbind(
        ...
        c(
          -(p - gamma(2 + xi)) / (sigma^2 * xi),
          (1 - 2 * gamma(2 + xi) + p) / (sigma^2 * xi^2),
          -(1 + digamma(1) + (1 - gamma(2 + xi)) / xi - q + p / xi) /
            (sigma * xi^2)
        ),
        c(
          (p / xi - q) /
    ...
    if (xi < 0.003 & xi > 0) {
      ...
      y2 <- (pi^2 /
        6 +
        (1 + digamma(1) + 1 / 0.003)^2 -
        2 *
          (gamma(2 + 0.003) *
            (digamma(1 + 0.003) +
              (1 + 0.003) / 0.003)) /
          0.003 +
        (1 + 0.003)^2 * gamma(1 + 2 * 0.003) / 0.003^2) /
        0.003^2
      y1 <- 2.42360605517703
    ...
    if (!isTRUE(all.equal(xi, 0, check.attributes = FALSE))) {
      infomat[1, 1] <- sum(
        -((dat - mu) * xi / sigma + 1)^(-1 / xi - 2) *
          (xi + 1) /
          sigma^2 +
          xi^2 *
            (1 / xi + 1) /
            (sigma^2 * ((dat - mu) * xi / sigma + 1)^2)
      )
      infomat[1, 2] <- infomat[2, 1] <- sum(
        -(dat - mu) *
          ((dat - mu) * xi / sigma + 1)^(-1 / xi - 2) *
          (xi + 1) /
          sigma^3 +
          ((dat - mu) * xi / sigma + 1)^(-1 / xi - 1) / sigma^2 -
          xi * (1 / xi + 1) / (sigma^2 * ((dat - mu) * xi / sigma + 1)) +
          (dat - mu) *
            xi^2 *
            (1 / xi + 1) /
            (sigma^3 *
      ...
      infomat[1, 3] <- infomat[3, 1] <- sum(
        (-(mu - dat) * xi / sigma + 1)^(-1 / xi - 1) *
          ((mu - dat) *
            ...
            log1p(-(mu - dat) * xi / sigma) / xi^2) /
          sigma -
          (1 / xi + 1) / (sigma * ((mu - dat) * xi / sigma - 1)) +
          (mu - dat) *
            (xi + 1) /
            (sigma^2 *
              ((mu -
                ...
                sigma -
                1)^2) +
          1 / (sigma * ((mu - dat) * xi / sigma - 1) * xi)
      )
      infomat[2, 3] <- infomat[3, 2] <- sum(
        (mu - dat) *
          (-(mu - dat) * xi / sigma + 1)^(-1 / xi - 1) *
          (log1p(-(mu - dat) * xi / sigma) /
            xi^2 -
          ...
          (mu - dat) *
            (1 / xi + 1) /
            (sigma^2 * ((mu - dat) * xi / sigma - 1)) -
          (mu - dat)^2 *
            (xi + 1) /
            (sigma^3 * ((mu - dat) * xi / sigma - 1)^2) -
          (mu - dat) /
            (sigma^2 *
              ((mu - dat) * xi / sigma - 1) *
              xi) +
      ...
      infomat[3, 3] <- sum(
        -(log1p(-(mu - dat) * xi / sigma) /
          ...
          (2 *
            ...
            xi^3 -
            2 * (mu - dat) / (sigma * ((mu - dat) * xi / sigma - 1) * xi^2) -
            (mu -
              dat)^2 /
              (sigma^2 * ((mu - dat) * xi / sigma - 1)^2 * xi)) /
            (-(mu - dat) * xi / sigma + 1)^(1 / xi) +
          (mu - dat)^2 *
            (1 / xi + 1) /
            (sigma^2 * ((mu - dat) * xi / sigma - 1)^2) -
          2 *
            log1p(
              -(mu -
                dat) *
      ...
      infomat[2, 2] <- sum(
        -2 *
          ...
          exp(mu / sigma - dat / sigma) /
          sigma^3 +
          2 * dat * exp(mu / sigma - dat / sigma) / sigma^3 +
          2 * mu / sigma^3 +
          1 / sigma^2 -
          2 * dat / sigma^3 -
          (mu^2 *
            exp(mu / sigma) -
            2 * mu * dat * exp(mu / sigma) +
            dat^2 * exp(mu / sigma)) *
            exp(-dat / sigma) /
            sigma^4
      )
      infomat[1, 3] <- infomat[3, 1] <- sum(
        -1 /
          ...
          (mu^2 *
            exp(mu / sigma) +
            2 * mu * sigma * exp(mu / sigma) -
            2 * mu * dat * exp(mu / sigma) -
            2 * sigma * dat * exp(mu / sigma) +
            dat^2 * exp(mu / sigma) -
            2 * mu * sigma * exp(dat / sigma) -
            2 * sigma^2 * exp(dat / sigma) +
            2 * sigma * dat * exp(dat / sigma)) *
          exp(-dat / sigma) /
          sigma^3
      )
      infomat[2, 3] <- infomat[3, 2] <- sum(
      ...
      infomat[3, 3] <- sum(
        exp(-(dat / sigma)) *
          (4 *
            ...
            sigma *
            (2 * mu + 3 * sigma - 2 * dat) -
            exp(mu / sigma) * (3 * mu + 8 * sigma - 3 * dat) * (mu - dat)) *
          (mu - dat)^2
      ) /
        (12 * sigma^4)
    }
...
gev.Vfun <- function(par, dat) {
  cbind(
    ...
    (dat - par[1]) / par[2],
    par[2] *
      (-(par[1] - dat) * par[3] / par[2] + 1)^(-1 / par[3]) *
      (log1p(
        -(par[1] -
          dat) *
          par[3] /
        ...
        par[3]^2 -
        (par[1] - dat) /
          (par[2] * ((par[1] - dat) * par[3] / par[2] - 1) * par[3])) /
      (-(par[1] - dat) * par[3] / par[2] + 1)^(-1 / par[3] - 1)
  )
}
  ...
  if (!isTRUE(all.equal(xi, 0))) {
    rbind(
      ...
      -(mu - dat) *
        ...
        (xi + 1) /
        sigma^3 -
        (-(mu - dat) * xi / sigma + 1)^(-1 / xi - 1) / sigma^2 -
        xi *
          (1 / xi + 1) /
          (sigma^2 *
            ((mu - dat) * xi / sigma - 1)) +
      ...
      -(-(mu - dat) * xi / sigma + 1)^(-1 / xi - 1) *
        ...
        (1 / xi + 1) /
          (sigma *
            ((mu - dat) * xi / sigma - 1)) -
        (mu - dat) * (xi + 1) / (sigma^2 * ((mu - dat) * xi / sigma - 1)^2) -
        1 / (sigma * ((mu - dat) * xi / sigma - 1) * xi)
    ) %*%
      V
  } else {
    rbind(
      exp(-dat / sigma + mu / sigma) / sigma^2,
      (sigma * exp(dat / sigma) + (dat - mu - sigma) * exp(mu / sigma)) *
        exp(-dat / sigma) /
        sigma^3,
      1 /
        2 *
        (2 *
          ...
          2 * sigma^2 * exp(dat / sigma) +
          dat^2 * exp(mu / sigma) -
          2 * dat * mu * exp(mu / sigma) +
          mu^2 *
            exp(mu / sigma) -
          2 * dat * sigma * exp(mu / sigma) +
          2 * mu * sigma * exp(mu / sigma)) *
        exp(-dat / sigma) /
        sigma^3
    ) %*%
...
gpde.ll <- function(par, dat, m) {
  ...
  xi = as.vector(par[2])
  if (
    any(xi > 1, es < 0, min(1 + (m^xi - 1 + xi) / ((1 - xi) * es) * dat) < 0)
  ) {
    return(-1e+10)
  }
  xizero <- abs(xi) < 1e-9
        ...
        -((m^xi * log(m) + 1) *
          ...
          (m^xi + xi - 1) *
            ...
            ((xi -
              1) *
              xi) +
          log1p(-dat * (m^xi + xi - 1) / (es * (xi - 1))) / xi^2
      )
    )
  } else {
    c(
      sum((dat * log(m) + dat - es) / es^2),
      sum(
        1 /
          2 *
          ((dat^2 - dat * es) *
            log(m)^3 +
            (3 * dat^2 - 5 * dat * es + es^2) * log(m)^2 +
            dat^2 -
            4 * dat * es +
            2 * es^2 +
            (3 * dat^2 - 8 * dat * es + 2 * es^2) * log(m)) /
          (es^2 * log(m) + es^2)
      )
    )
  }
    ...
    if (!xizero) {
      ...
      k22 = sum(
        -((m^xi * logm + 1) *
          ...
          (m^xi *
            ...
            logm^2 /
            (m^xi + xi - 1)^2 -
            2 * (m^xi * logm + 1)^2 * (xi - 1) * xi / (m^xi + xi - 1)^3 +
            2 * (m^xi * logm + 1) * (xi - 1) / (m^xi + xi - 1)^2 +
            2 *
              (m^xi * logm + 1) *
              xi /
              (m^xi +
          ...
          2 *
            ((m^xi * logm + 1) *
              dat /
              (es * (xi - 1)) -
              dat * (m^xi + xi - 1) / (es * (xi - 1)^2)) /
            (xi^2 * (dat * (m^xi + xi - 1) / (es * (xi - 1)) - 1)) +
          2 * log1p(-dat * (m^xi + xi - 1) / (es * (xi - 1))) / xi^3
      )
      k12 = sum(
        -((m^xi * logm + 1) *
          dat /
      ...
      k22 <- sum(
        1 /
          12 *
          (4 *
            (2 * dat^3 - 3 * dat^2 * es + dat * es^2) *
            log(m)^5 +
            (40 * dat^3 - 72 * dat^2 * es + 32 * dat * es^2 - es^3) * log(m)^4 +
            4 *
              (20 * dat^3 - 45 * dat^2 * es + 25 * dat * es^2 - es^3) *
              log(m)^3 +
            8 * dat^3 -
            36 * dat^2 * es +
            48 * dat * es^2 -
            12 * es^3 +
            4 *
              (20 * dat^3 - 57 * dat^2 * es + 42 * dat * es^2 - 3 * es^3) *
              log(m)^2 +
            8 *
              (5 * dat^3 - 18 * dat^2 * es + 18 * dat * es^2 - 3 * es^3) *
              log(m)) /
          (es^3 * log(m)^2 + 2 * es^3 * log(m) + es^3)
      )
    }
  ...
  cbind(
    ...
    es *
      ...
      ((m^xi +
        xi -
        1) *
        (-dat * (m^xi + xi - 1) / (es * (xi - 1)) + 1)^(-1 / xi - 1))
  )
}
...
gpde.phi <- function(par, dat, V, m) {
  ...
  rbind(
    -(m^xi + xi - 1) *
      (1 / xi + 1) /
      (es * (xi - 1) * (dat * (m^xi + xi - 1) / (es * (xi - 1)) - 1))
  ) %*%
    V
}
...
gpde.dphi <- function(par, dat, V, m) {
  ...
  rbind(
    (m^xi + xi - 1) *
      (1 / xi + 1) /
      (es^2 * (xi - 1) * (dat * (m^xi + xi - 1) / (es * (xi - 1)) - 1)) -
      dat *
        (m^xi + xi - 1)^2 *
        (1 / xi + 1) /
        (es^3 *
    ...
    (m^xi + xi - 1) *
      ((m^xi * log(m) + 1) *
        ...
        dat *
          ...
          (es * (xi - 1)^2)) *
      (1 / xi + 1) /
      (es * (xi - 1) * (dat * (m^xi + xi - 1) / (es * (xi - 1)) - 1)^2) -
      (m^xi * log(m) + 1) *
        (1 / xi + 1) /
        (es *
          (xi - 1) *
      ...
      (m^xi + xi - 1) *
        ...
        (es *
          ...
          (dat *
            ...
            1)) +
      (m^xi + xi - 1) /
        (es * (xi - 1) * xi^2 * (dat * (m^xi + xi - 1) / (es * (xi - 1)) - 1))
  ) %*%
    V
}
...
gpdr.ll.optim <- function(par, dat, m) {
  ...
  if (abs(xi) > tol) {
    -(-nn *
      log(xi / p) -
      nn * log(ym) -
      (1 + 1 / xi) * sum(log(pmax(1 + exp(log(dat) - log(ym)) * p, 0))))
  } else {
    -(nn * log(log(m)) - nn * log(ym) + sum(-dat * log(m) / ym))
  }
}
...
gpdr.score <- function(par, dat, m) {
  ...
  if (!xizero) {
    as.vector(c(
      -nn / ym + (1 + 1 / xi) * sum(dat * p / (ym + dat * p)) / ym,
      -nn /
        xi +
        exp(log(nn) + xi * log(m) + log(log(m))) / p +
        sum(log1p(dat / ym * p)) / xi^2 -
        (1 + 1 / xi) *
          sum(exp(
            log(dat) +
              log(log(m)) +
    ...
    as.vector(c(
      sum((dat * log(m) - ym)) / ym^2,
      0.5 *
        sum(
          (dat^2 *
            log(m)^2 +
            ym^2 * log(m) -
            (dat * log(m)^2 + 2 * dat * log(m)) * ym)
        ) /
        ym^2
    ))
  }
...
gpdr.infomat <- function(
  ...
  if (method == "obs") {
    if (!xizero) {
      ...
      info[2, 2] <- sum(
        dat^2 *
          ...
          (m^xi *
            ...
            logm^2 /
            (m^xi - 1)^2 -
            2 * (m^xi)^2 * xi * logm^2 / (m^xi - 1)^3 +
            2 * m^xi * logm / (m^xi - 1)^2) *
            (m^xi - 1) /
            xi -
          (m^xi - 1) *
            (m^xi *
              ...
              1 / (m^xi - 1)) /
            xi^2 +
          2 * dat * m^xi * logm / ((dat * (m^xi - 1) / r + 1) * r * xi^2) -
          2 * log1p(dat * (m^xi - 1) / r) / xi^3
      )
      info[2, 1] <- info[1, 2] <- sum(
        -dat^2 *
          (m^xi - 1) *
          m^xi *
          (1 / xi + 1) *
      ...
      k12 <- 0.5 *
        sum((2 * dat * log(m) - r * log(m) - 2 * r) * dat) *
        log(m) /
        r^3
      k22 <- sum(
        (8 *
          ...
          log(m)^3 -
          r^3 * log(m)^2 +
          4 * (dat * log(m)^3 + 3 * dat * log(m)^2) * r^2 -
          12 * (dat^2 * log(m)^3 + dat^2 * log(m)^2) * r) /
          (12 * r^3)
      )
    ...
    if (!xizero) {
      Jac <- rbind(
        c(
          xi / (m^xi - 1),
          -m^xi * r * xi * logm / (m^xi - 1)^2 + r / (m^xi - 1)
        ),
        c(0, 1)
      )
    } else {
...
gpdr.Vfun <- function(par, dat, m) {
  ...
  cbind(
    ...
    -(ym + dat * p) /
      p *
      ((m)^xi * log(m) / (p + ym / dat) - log1p(dat / ym * p) / xi)
  )
}
...
gevr.ll <- function(par, dat, p) {
  ...
  if (!isTRUE(all.equal(xi, 0, tolerance = 1e-08))) {
    sum(
      -log(sigma) -
        (1 / xi + 1) * log(pmax(1 + xi * (dat - muf) / sigma, 0)) -
        pmax(
          1 +
            xi *
              (dat -
...
gevr.score <- function(par, dat, p) {
  ...
  if (abs(xi) > 1e-4) {
    c(
      sum(
        (((dat - z) * xi / sigma + 1 / (-log1mp)^xi)^(-1 / xi - 2) *
          (xi + 1) *
          exp(
            -1 /
              ((dat - z) *
          ...
          sigma^2 -
          ((dat - z) * xi / sigma + 1 / (-log1mp)^xi)^(-2 / xi - 2) *
            exp(-1 / ((dat - z) * xi / sigma + 1 / (-log1mp)^xi)^(1 / xi)) /
            sigma^2) *
          sigma *
          ((dat - z) *
            xi /
      ...
      sum(
        -(dat *
          (-log1mp)^xi -
          z * (-log1mp)^xi -
          (dat * (-log1mp)^xi - z * (-log1mp)^xi - sigma) *
            ((dat * xi * (-log1mp)^xi - xi * z * (-log1mp)^xi + sigma) /
              (sigma *
                (-log1mp)^xi))^(1 / xi)) /
          ((sigma *
            dat *
      ...
      sum(
        -(xi *
          z *
          (-log1mp)^xi -
          (dat * (-log1mp)^xi - sigma * log(-log1mp)) * xi +
          ((dat * (-log1mp)^xi - sigma * log(-log1mp)) *
            xi^2 +
            (dat * (-log1mp)^xi - sigma * log(-log1mp)) * xi -
            (xi^2 * (-log1mp)^xi + xi * (-log1mp)^xi) * z) *
            ((dat * xi * (-log1mp)^xi - xi * z * (-log1mp)^xi + sigma) /
              (sigma * (-log1mp)^xi))^(1 / xi) +
          (dat *
            xi *
            (-log1mp)^xi -
            xi * z * (-log1mp)^xi -
    ...
    as.vector(c(
      ...
      sum((-sigma + dat - z + exp((-dat + z) / sigma) * (dat - z) * log1mp)) /
        sigma^2,
      sum(
        (1 / (2 * sigma^2)) *
          ((exp(z / sigma) *
            (-dat + z) *
            log1mp *
            (-dat + z + 2 * sigma * log(-log1mp)) +
            exp(dat / sigma) *
              ((dat - z) *
                (-2 * sigma + dat - z) +
                2 * sigma * (sigma - dat + z) * log(-log1mp))) /
            exp(dat / sigma))
      )
    ))
  }
...
gevr.infomat <- function(
  ...
  if (method == "obs") {
    ...
    if (abs(xi) > 1e-4) {
      info[1, 1] <- sum(
        -(xi *
          ...
          ((dat^2 *
            xi^2 *
            (-log1mp)^(2 * xi) +
            xi^2 * z^2 * (-log1mp)^(2 * xi) +
            2 * sigma * dat * xi * (-log1mp)^xi +
            sigma^2 -
            2 *
              (dat * xi^2 * (-log1mp)^(2 * xi) + sigma * xi * (-log1mp)^xi) *
              z) *
            ((dat * xi * (-log1mp)^xi - xi * z * (-log1mp)^xi + sigma) /
              (sigma * (-log1mp)^xi))^(1 / xi))
      )

      info[1, 2] <- info[2, 1] <- sum(
        -(dat *
          (-log1mp)^(2 * xi) -
          z *
            (-log1mp)^(2 *
              xi) -
          sigma * (-log1mp)^xi +
          (sigma * xi * (-log1mp)^xi + sigma * (-log1mp)^xi) *
            ((dat * xi * (-log1mp)^xi - xi * z * (-log1mp)^xi + sigma) /
              (sigma * (-log1mp)^xi))^(1 / xi)) /
          ((sigma *
            dat^2 *
            xi^2 *
            (-log1mp)^(2 * xi) +
            sigma * xi^2 * z^2 * (-log1mp)^(2 * xi) +
            2 * sigma^2 * dat * xi * (-log1mp)^xi +
            sigma^3 -
            2 *
              (sigma *
                dat *
                xi^2 *
                (-log1mp)^(2 * xi) +
                sigma^2 * xi * (-log1mp)^xi) *
              z) *
            ((dat * xi * (-log1mp)^xi - xi * z * (-log1mp)^xi + sigma) /
              (sigma * (-log1mp)^xi))^(1 / xi))
      )
      info[1, 3] <- info[3, 1] <- sum(
        -((sigma *
          (-log1mp)^xi *
          logmlog1mp -
          dat *
          ...
          ((dat^2 *
            xi^4 *
            (-log1mp)^(2 * xi) +
            xi^4 * z^2 * (-log1mp)^(2 * xi) +
            2 * sigma * dat * xi^3 * (-log1mp)^xi +
            sigma^2 *
              xi^2 -
            2 *
              (dat * xi^4 * (-log1mp)^(2 * xi) + sigma * xi^3 * (-log1mp)^xi) *
              z) *
            ((dat * xi * (-log1mp)^xi - xi * z * (-log1mp)^xi + sigma) /
              (sigma * (-log1mp)^xi))^(1 / xi))
      )
      info[2, 2] <- sum(
        (dat^2 *
          xi *
          (-log1mp)^(2 * xi) +
          (xi * (-log1mp)^(2 * xi) - (-log1mp)^(2 * xi)) * z^2 -
          dat^2 * (-log1mp)^(2 * xi) +
          2 * sigma * dat * (-log1mp)^xi -
          2 *
            (dat *
              xi *
              (-log1mp)^(2 * xi) -
          ...
          ((sigma^2 *
            ...
            xi^2 *
            (-log1mp)^(2 * xi) +
            sigma^2 * xi^2 * z^2 * (-log1mp)^(2 * xi) +
            2 * sigma^3 * dat * xi * (-log1mp)^xi +
            sigma^4 -
            2 *
              (sigma^2 *
                dat *
      ...
      info[3, 2] <- info[2, 3] <- sum(
        -((sigma *
          ...
          ((sigma *
            ...
            xi^2 +
            (sigma * (-log1mp)^xi * logmlog1mp - 2 * dat * (-log1mp)^(2 * xi)) *
              xi) *
            z -
          (sigma *
            dat *
          ...
          (dat^2 *
            ...
            xi * z^2 * (-log1mp)^(2 * xi) +
            sigma * dat * (-log1mp)^xi -
            (2 * dat * xi * (-log1mp)^(2 * xi) + sigma * (-log1mp)^xi) * z) *
            log(
              (dat * xi * (-log1mp)^xi - xi * z * (-log1mp)^xi + sigma) /
                (sigma * (-log1mp)^xi)
            )) /
          ((sigma *
            ...
            (-log1mp)^(2 *
              xi) +
            sigma * xi^4 * z^2 * (-log1mp)^(2 * xi) +
            2 * sigma^2 * dat * xi^3 * (-log1mp)^xi +
            sigma^3 * xi^2 -
            2 *
              (sigma *
                dat *
      ...
      info[3, 3] <- sum(
        (sigma *
          ...
          (2 *
            ...
            dat^2 * (-log1mp)^(2 * xi)) *
            xi^2 -
          (3 * xi^3 * (-log1mp)^(2 * xi) + xi^2 * (-log1mp)^(2 * xi)) * z^2 -
          (dat^2 *
            xi^2 *
            (-log1mp)^(2 * xi) +
            xi^2 * z^2 * (-log1mp)^(2 * xi) +
          ...
          (sigma *
            ...
            (4 *
              ...
              3 * dat^2 * (-log1mp)^(2 * xi)) *
              xi^3 -
            2 * (sigma * dat * (-log1mp)^xi - sigma^2 * logmlog1mp) * xi^2 -
            (xi^4 * (-log1mp)^(2 * xi) + 3 * xi^3 * (-log1mp)^(2 * xi)) *
              z^2 -
            (sigma *
              xi^5 *
              (-log1mp)^xi *
          ...
          2 *
            (dat^2 *
              ...
              (sigma *
                ...
                dat^2 * (-log1mp)^(2 * xi)) *
                xi^2 +
              (xi^3 * (-log1mp)^(2 * xi) + xi^2 * (-log1mp)^(2 * xi)) * z^2 +
              (sigma *
                dat *
                (-log1mp)^xi -
                sigma^2 *
          ...
          ((dat^2 *
            ...
            xi^6 *
              ...
              (-log1mp)^(2 *
                xi) +
            2 * sigma * dat * xi^5 * (-log1mp)^xi +
            sigma^2 * xi^4 -
            2 *
              (dat * xi^6 * (-log1mp)^(2 * xi) + sigma * xi^5 * (-log1mp)^xi) *
              z) *
            ((dat * xi * (-log1mp)^xi - xi * z * (-log1mp)^xi + sigma) /
              (sigma * (-log1mp)^xi))^(1 / xi))
      )
      ...
      info[1, 2] <- sum(
        -((sigma + exp((-dat + z) / sigma) * (sigma - dat + z) * log1mp))
      ) /
        sigma^3
      info[2, 1] <- info[1, 2]
      info[1, 3] <- sum(
        (1 / (2 * sigma^3)) *
          ((2 *
            exp(dat / sigma) *
            sigma *
            (sigma - dat + z + sigma * logmlog1mp) +
            exp(z / sigma) *
              log1mp *
              ((dat - z) *
                (-2 * sigma + dat - z) +
                2 * sigma * (sigma - dat + z) * logmlog1mp)) /
            exp(dat / sigma))
      )
      info[3, 1] <- info[1, 3]
      info[2, 2] <- sum(
        sigma *
          (sigma - 2 * dat + 2 * z) +
          exp((-dat + z) / sigma) * (dat - z) * (-2 * sigma + dat - z) * log1mp
      ) /
        sigma^4
      info[2, 3] <- sum(
        (1 / (2 * sigma^4)) *
            ...
            (4 *
              ...
              ((dat - z) *
                ...
                3 *
                  sigma *
                  logmlog1mp *
                  (2 * (sigma - dat + z) + sigma * logmlog1mp)) +
              exp((-dat + z) / sigma) *
                log1mp *
                ((-8 * sigma + 3 * dat - 3 * z) *
                  (dat - z)^2 +
    ...
    if (
      !isTRUE(all.equal(xi, 0, tolerance = 1e-07, check.attributes = FALSE))
    ) {
      Jac <- rbind(
        c(
          1,
      ...
      Jac <- rbind(
        c(
          ...
          -sigma *
            log(-log1mp)^2 +
            1 / 2 * sigma * (-log1mp)^(-xi) * log(-log1mp)^2
        ),
        c(0, 1, 0),
        c(0, 0, 1)
      )
    ...
    infomat <- t(Jac) %*%
      gev.infomat(dat = dat, method = "exp", par = c(muf, sigma, xi)) %*%
      Jac
  }
...
gevr.Vfun <- function(par, dat, p) {
  ...
  cbind(
    ...
    (dat - z) / sigma,
    sigma *
      ((dat - z) * xi / sigma + (-log1mp)^(-xi))^(-1 / xi) *
      (((-log1mp)^(-xi) * log(-log1mp) - (dat - z) / sigma) /
        (((dat - z) * xi / sigma + (-log1mp)^(-xi)) *
          xi) +
        log((dat - z) * xi / sigma + (-log1mp)^(-xi)) / xi^2) /
      ((dat - z) * xi / sigma + (-log1mp)^(-xi))^(-1 / xi - 1)
  )
}
  ...
  rbind(
    ...
    (dat *
      ...
      ((sigma *
        ...
        ((dat * xi * (-log1mp)^xi - xi * z * (-log1mp)^xi + sigma) /
          (sigma * (-log1mp)^xi))^(1 / xi)),
    ((sigma * (-log1mp)^xi * log(-log1mp) - dat * (-log1mp)^(2 * xi)) *
      xi^2 +
      (sigma *
        (-log1mp)^xi *
        log(-log1mp) -
...
gpdN.score <- function(par, dat, N) {
  ...
  if (!xizero) {
    ...
    as.vector(c(
      sum(dat * euler_gamma - z + dat * psi1pN) / z^2,
      sum(
        (6 *
          dat^2 *
          euler_gamma^3 -
          12 * dat * euler_gamma^2 * z -
          6 * dat * euler_gamma^3 * z -
          dat * euler_gamma * pi^2 * z +
          6 * euler_gamma^2 * z^2 +
          pi^2 * z^2 +
          6 *
            (3 * dat^2 * euler_gamma - dat * (2 + 3 * euler_gamma) * z + z^2) *
            psigamma(1 + N)^2 +
          6 * dat * (dat - z) * psigamma(1 + N)^3 +
          6 * (dat * euler_gamma - z) * z * psigamma(deriv = 1, 1 + N) +
          psigamma(1 + N) *
            (18 *
              dat^2 *
              euler_gamma^2 -
              dat * (24 * euler_gamma + 18 * euler_gamma^2 + pi^2) * z +
              12 * euler_gamma * z^2 +
              6 * dat * z * psigamma(deriv = 1, 1 + N))) /
          (12 * z^2 * (euler_gamma + psigamma(1 + N)))
      )
    ))
  }
}
  ...
  if (method == "exp") {
    if (!xizero) {
      ...
      Jac <- rbind(
        c(
          xi / (cst - 1),
          -(digamma(N - xi + 1) * cst - digamma(-xi + 1) * cst) *
            xi *
            z /
            (cst - 1)^2 +
            z / (cst - 1)
      ...
      Jac <- rbind(
        c(
          1 / (euler_gamma + psi1pN),
          -(z *
            (6 *
              euler_gamma^2 +
              pi^2 +
              12 * euler_gamma * psigamma(1 + N) +
              6 * psigamma(1 + N)^2 -
              6 * psigamma(1 + N, deriv = 1))) /
            (12 * (euler_gamma + psigamma(1 + N))^2)
        ),
        c(0, 1)
      )
    }
    infomat <- t(Jac) %*%
      gpd.infomat(par = c(sigmaf, xi), dat = dat, method = "exp") %*%
      Jac
  } else if (method == "obs") {
    ...
    if (!xizero) {
      ...
      k22 <- sum(
        -(digamma(N - xi + 1) * cst - digamma(-xi + 1) * cst)^2 *
          dat^2 *
          (1 / xi + 1) /
          (z^2 *
            (dat * (cst - 1) / z + 1)^2) +
          (digamma(N - xi + 1)^2 *
            ...
            2 *
              ...
              cst +
            digamma(-xi + 1)^2 * cst -
            psigamma(deriv = 1, N - xi + 1) * cst +
            psigamma(deriv = 1, -xi + 1) * cst) *
            dat *
            (1 / xi + 1) /
            (z * (dat * (cst - 1) / z + 1)) -
          (digamma(N - xi + 1) *
        ...
        (1 / (12 * z^3)) *
          (dat *
            (-12 *
              dat *
              euler_gamma^2 +
              (6 * euler_gamma * (2 + euler_gamma) + pi^2) * z +
              12 *
                (-2 * dat * euler_gamma + z + euler_gamma * z) *
                psigamma1pN +
              6 * (-2 * dat + z) * psigamma1pN^2 -
              6 * z * psigammap1pN))
      )
      k22 <- -sum(
        (-96 *
          ...
          24 *
            dat^2 *
            euler_gamma^3 *
            (6 * euler_gamma * (1 + euler_gamma) + pi^2) *
            z -
          24 *
            dat *
            euler_gamma^2 *
            (2 * euler_gamma^2 * (3 + euler_gamma) + (1 + euler_gamma) * pi^2) *
            z^2 +
          (12 * euler_gamma^4 + 12 * euler_gamma^2 * pi^2 - pi^4) * z^3 -
          12 *
            ((40 *
              dat^3 *
              euler_gamma +
              4 * dat * (3 + 5 * euler_gamma) * z^2 -
              z^3 -
              12 * dat^2 * (z + 5 * euler_gamma * z)) *
              psigamma1pN^4 +
              ...
              2 *
                ...
                (40 *
                  ...
                  euler_gamma^2 -
                  dat^2 *
                    (12 * euler_gamma * (2 + 5 * euler_gamma) + pi^2) *
                    z +
                  dat * (4 * euler_gamma * (6 + 5 * euler_gamma) + pi^2) * z^2 -
                  2 * euler_gamma * z^3 +
                  6 * dat * (dat - z) * z * psigammap1pN) +
              z *
                ((12 *
                  dat^2 *
                  euler_gamma^3 -
                  12 * dat * euler_gamma^2 * (1 + euler_gamma) * z +
                  (6 * euler_gamma^2 - pi^2) * z^2) *
                  psigammap1pN +
                  3 * z^2 * psigammap1pN^2 +
                  4 *
                    euler_gamma *
                    (dat * euler_gamma - z) *
                    z *
                    (psigammapp1pN + 2 * zeta3)) +
              psigamma1pN^2 *
                (80 *
                  dat^3 *
                  euler_gamma^3 -
                  6 *
                    dat^2 *
                    euler_gamma *
                    (4 * euler_gamma * (3 + 5 * euler_gamma) + pi^2) *
                    z +
                  2 *
                    dat *
                    (4 *
                      euler_gamma^2 *
                      (9 + 5 * euler_gamma) +
                      (1 + 3 * euler_gamma) * pi^2) *
                    z^2 -
                  (6 * euler_gamma^2 + pi^2) * z^3 +
                  6 *
                    z *
                    (6 *
                      dat^2 *
                      euler_gamma -
                      2 * dat * (1 + 3 * euler_gamma) * z +
                      z^2) *
                    psigammap1pN +
                  4 * dat * z^2 * (psigammapp1pN + 2 * zeta3)) +
              2 *
                psigamma1pN *
                (euler_gamma *
                  (20 *
                    ...
                    3 *
                      dat^2 *
                      euler_gamma *
                      (2 * euler_gamma * (4 + 5 * euler_gamma) + pi^2) *
                      z +
                    dat *
                      (2 *
                        euler_gamma^2 *
                        (12 + 5 * euler_gamma) +
                        (2 + 3 * euler_gamma) * pi^2) *
                      z^2 -
                    (2 * euler_gamma^2 + pi^2) * z^3) +
                  6 *
                    euler_gamma *
                    z *
                    (3 *
                  ...
                  2 *
                    ...
                    (-2 * dat * euler_gamma + z) *
                    (psigammapp1pN + 2 * zeta3)))) /
          (144 * z^3 * (psigamma1pN + euler_gamma)^2)
      )
    }
    infomat <- cbind(c(k11, k12), c(k12, k22))
  }
...
gevN.score <- function(par, dat, N, q = 0.5, qty = c("mean", "quantile")) {
  ...
  if (qty == "quantile") {
    ...
    if (abs(xi) > 1e-4) {
      as.vector(c(
        sum(
          (-(N^xi * log1q^(-xi) - 1) * (dat - mu) / (mu - z) + 1)^(-1 /
            xi -
            1) *
            ((N^xi * log1q^(-xi) - 1) *
              (dat - mu) /
              (mu - z)^2 +
              (N^xi * log1q^(-xi) - 1) / (mu - z)) /
            xi +
            ((N^xi * log1q^(-xi) - 1) *
              (dat - mu) /
              (mu - z)^2 +
        ...
        sum(
          -(N^xi * log1q^(-xi) - 1) *
            ...
            (N^xi * log1q^(-xi) - 1) *
              (dat - mu) *
              (1 / xi + 1) /
              (((N^xi * log1q^(-xi) - 1) * (dat - mu) / (mu - z) - 1) *
                (mu - z)^2) +
            1 / (mu - z)
        ),
        sum(
          (-(N^xi * log1q^(-xi) - 1) * (dat - mu) / (mu - z) + 1)^(-1 / xi) *
            ((N^xi * log1q^(-xi) * log(N) - N^xi * log1q^(-xi) * log(log1q)) *
              (dat - mu) /
              (((N^xi * log1q^(-xi) - 1) * (dat - mu) / (mu - z) - 1) *
                (mu -
                  z) *
                xi) -
              log(-(N^xi * log1q^(-xi) - 1) * (dat - mu) / (mu - z) + 1) /
                xi^2) -
            (N^xi * log1q^(-xi) * log(N) - N^xi * log1q^(-xi) * log(log1q)) *
              (dat - mu) *
              (1 / xi + 1) /
            ...
            (N^xi * log1q^(-xi) - 1) *
              ((N^xi * log1q^(-xi) * log(N) - N^xi * log1q^(-xi) * log(log1q)) *
                ...
                xi /
                (N^xi * log1q^(-xi) - 1)^2 -
                (mu - z) / (N^xi * log1q^(-xi) - 1)) /
              ((mu - z) * xi) +
            log1p(-(N^xi * log1q^(-xi) - 1) * (dat - mu) / (mu - z)) / xi^2
        )
      ))
    } else {
      logN <- log(N)
      ...
      as.vector(c(
        sum(
          (-mu +
            z +
            (-1 + exp(((mu - dat) * (-logN + loglog1q)) / (mu - z))) *
              (dat - z) *
              (logN - loglog1q)) /
            (mu - z)^2
        ),
        sum(
          (mu -
            ...
            (mu - dat) *
              (-1 +
                N^((-mu + dat) / (mu - z)) * log1q^((mu - dat) / (mu - z))) *
              (logN - loglog1q)) /
            (mu - z)^2
        ),
        sum(
    ...
    if (abs(xi) > 1e-4) {
      as.vector(c(
        sum(
          (-(N^xi * gamma(-xi + 1) - 1) * (dat - mu) / (mu - z) + 1)^(-1 /
            xi -
            1) *
            ((N^xi *
              gamma(
          ...
          (-(N^xi *
            ...
            ((N^xi *
              ...
              N^xi *
                ...
                gamma(-xi + 1)) *
              (dat - mu) /
              (((N^xi * gamma(-xi + 1) - 1) * (dat - mu) / (mu - z) - 1) *
                (mu - z) *
                xi) -
              log1p(-(N^xi * gamma(-xi + 1) - 1) * (dat - mu) / (mu - z)) /
                xi^2) -
            (N^xi *
              log(N) *
              gamma(-xi + 1) -
              N^xi * psigamma(-xi + 1) * gamma(-xi + 1)) *
              (dat - mu) *
              (1 / xi + 1) /
              (((N^xi * gamma(-xi + 1) - 1) * (dat - mu) / (mu - z) - 1) *
                (mu - z)) +
            (N^xi *
              ...
              ((N^xi *
                ...
                xi /
                (N^xi * gamma(-xi + 1) - 1)^2 -
                (mu - z) / (N^xi * gamma(-xi + 1) - 1)) /
              ((mu -
                z) *
                xi) +
            log1p(-(N^xi * gamma(-xi + 1) - 1) * (dat - mu) / (mu - z)) / xi^2
        )
      ))
    } else {
      logN <- log(N)
      ...
      as.vector(c(
        sum(
          (1 / (mu - z)^2) *
            (-mu -
              ...
              dat * log(N) +
              z * log(N) +
              exp(((mu - dat) * (euler_gamma + log(N))) / (-mu + z)) *
                (dat -
                  z) *
                (euler_gamma + log(N)))
        ),
        ...
        sum(
          (exp((mu * (euler_gamma + log(N))) / (-mu + z)) *
            (exp((dat * (euler_gamma + log(N))) / (mu - z)) *
              (mu - dat) *
              (euler_gamma + log(N)) *
              (pi^2 *
                (mu - z) +
                6 * euler_gamma^2 * (dat - z) +
                6 * (dat - z) * log(N) * (2 * euler_gamma + log(N))) +
              exp((mu * (euler_gamma + log(N))) / (mu - z)) *
                ((-euler_gamma) *
                  pi^2 *
                  (mu - dat) *
                  ...
                  6 * euler_gamma^2 * (mu - z) * (mu - 2 * dat + z) +
                  log(N) *
                    ((-pi^2) *
                      (mu - dat) *
                      (mu - z) -
                      18 * euler_gamma^2 * (mu - dat) * (dat - z) -
                      12 * euler_gamma * (mu - z) * (mu - 2 * dat + z) +
                      6 *
                        log(N) *
                        (-mu^2 +
                          3 * euler_gamma * dat * (dat - z) +
                          z * (-2 * dat + z) +
                          mu *
                            ((2 - 3 * euler_gamma) *
                              dat +
...
gevN.infomat <- function(
  ...
  if (qty == "mean") {
    ...
    if (method == "obs") {
      if (!xizero) {
        k11 <- sum(
          ((N^xi * gamma(-xi + 1) - 1) * (mu - dat) / (mu - z) + 1)^(-1 /
            xi -
            2) *
            ((N^xi *
              gamma(-xi + 1) -
            ...
            ((N^xi * gamma(-xi + 1) - 1) *
              ...
              (N^xi *
                ...
                (mu - z))^2 *
              (1 / xi + 1) /
              ((N^xi * gamma(-xi + 1) - 1) * (mu - dat) / (mu - z) + 1)^2 +
            2 *
              ((N^xi * gamma(-xi + 1) - 1) *
                (mu - dat) /
                (mu - z)^3 -
                (N^xi *
                  ...
                  (mu - z)^2) *
              (1 / xi + 1) /
              ((N^xi * gamma(-xi + 1) - 1) * (mu - dat) / (mu - z) + 1) -
            1 / (mu - z)^2
        )
        k12 <- sum(
          -(N^xi * gamma(-xi + 1) - 1) *
          ...
          (N^xi * gamma(-xi + 1) - 1) *
            ...
            ((N^xi *
              ...
              (((N^xi * gamma(-xi + 1) - 1) *
                ...
                1) *
                (mu - z)) -
              log1p((N^xi * gamma(-xi + 1) - 1) * (mu - dat) / (mu - z)) /
                xi^2) /
            ((mu - z)^2 * xi) -
            (N^xi *
              logN *
        ...
        k33 <- sum(
          ((N^xi * gamma(-xi + 1) - 1) * (mu - dat) / (mu - z) + 1)^(-1 / xi) *
            ((N^xi *
              logN *
              gamma(-xi + 1) -
              N^xi * psigamma(-xi + 1) * gamma(-xi + 1)) *
            ...
            ((N^xi * gamma(-xi + 1) - 1) * (mu - dat) / (mu - z) + 1)^(-1 /
              ...
              ((N^xi *
                ...
                (N^xi *
                  logN^2 *
                  gamma(-xi + 1) -
                  2 * N^xi * logN * psigamma(-xi + 1) * gamma(-xi + 1) +
                  N^xi * psigamma(-xi + 1)^2 * gamma(-xi + 1) +
                  N^xi *
                    psigamma(
                      1,
                      -xi +
            ...
            (N^xi *
              logN *
              gamma(-xi + 1) -
              N^xi * psigamma(-xi + 1) * gamma(-xi + 1))^2 *
              (mu -
                dat)^2 *
              (1 / xi + 1) /
              (((N^xi * gamma(-xi + 1) - 1) * (mu - dat) / (mu - z) + 1)^2 *
              ...
              (2 *
                ...
                (N^xi *
                  ...
                  2 *
                    ...
                    gamma(-xi + 1) +
                  N^xi * psigamma(-xi + 1)^2 * gamma(-xi + 1) +
                  N^xi * psigamma(1, -xi + 1) * gamma(-xi + 1)) *
                  (mu - z) *
                  xi /
                  (N^xi * gamma(-xi + 1) - 1)^2 -
                2 *
        ...
        k11 <- -sum(
          (1 / (mu - z)^4) *
            ((-exp(((mu - dat) * (euler_gamma + logN)) / (-mu + z))) *
              (dat - z) *
              (euler_gamma + logN) *
              (2 *
                mu +
            ...
            (24 *
              ((-(mu - dat)) *
                ...
                (2 *
                  ...
                  (mu - z) -
                  pi^2 * (mu - z)^2 +
                  30 * euler_gamma^3 * (mu - dat) * (dat - z) +
                  20 * euler_gamma^2 * (mu - z) * (mu - 2 * dat + z)) +
                exp(
                  ((mu - dat) * (euler_gamma + logN)) /
                    (mu - z)
                ) *
                  (mu - z) *
                  ((-pi^2) *
              ...
              12 *
                ((-(mu - dat)) *
                  ...
                  (pi^2 *
                    (mu - dat) *
                    (mu - z) +
                    45 * euler_gamma^2 * (mu - dat) * (dat - z) +
                    20 * euler_gamma * (mu - z) * (mu - 2 * dat + z)) +
                  exp(((mu - dat) * (euler_gamma + logN)) / (mu - z)) *
                    (mu - z) *
                    (mu^3 +
                      40 * euler_gamma * dat^3 -
                      12 * (1 + 5 * euler_gamma) * dat^2 * z +
                      4 * (3 + 5 * euler_gamma) * dat * z^2 -
                      z^3 +
                      mu^2 *
                        (4 *
                          (-3 + 5 * euler_gamma) *
                      ...
                      mu *
                        ((12 - 60 * euler_gamma) *
                          dat^2 +
                          80 * euler_gamma * dat * z -
                          (9 + 20 * euler_gamma) * z^2))) *
                logN^4 +
              24 *
                (mu - dat) *
                (dat - z) *
                (-2 *
                  ...
                  2 *
                    ...
                    (mu - 2 * dat + z)) *
                logN^5 -
              36 * (mu - dat)^2 * (dat - z)^2 * logN^6 -
              euler_gamma^2 *
                (mu - dat) *
                (mu^3 *
                  pi^4 +
                  48 * euler_gamma^3 * mu^2 * (dat - z) +
                  12 * euler_gamma^2 * mu^2 * pi^2 * (dat - z) +
                  36 * euler_gamma^4 * mu * (dat - z)^2 -
                  36 * euler_gamma^4 * dat * (dat - z)^2 +
                  48 * euler_gamma * mu * pi^2 * (dat - z) * z +
                  12 * euler_gamma^2 * pi^2 * dat * (dat - z) * z +
                  48 * euler_gamma^3 * (dat - z) * (2 * dat - z) * z -
                  pi^4 * dat * z^2 +
                  24 * euler_gamma * mu^2 * pi^2 * (-dat + z) +
                  96 * euler_gamma^3 * mu * dat * (-dat + z) +
                  24 * euler_gamma * pi^2 * z^2 * (-dat + z) -
                  12 * euler_gamma^2 * mu * pi^2 * (dat - z) * (dat + z) +
                  mu * pi^4 * z * (2 * dat + z) -
                  mu^2 * pi^4 * (dat + 2 * z) -
                  96 * (mu - z)^3 * zeta3) +
              exp(((mu - dat) * (euler_gamma + logN)) / (mu - z)) *
                ...
                ((-pi^4) *
                  ...
                  12 *
                    ...
                    (mu^2 +
                      ...
                      z^2 +
                      2 * mu * (-6 * dat + 5 * z)) +
                  96 * euler_gamma * (mu - z)^3 * zeta3 -
                  12 *
                    euler_gamma^2 *
                    (mu - z)^2 *
                    (pi^2 * (mu - 2 * dat + z) + 8 * (mu - dat) * zeta3)) +
              logN^2 *
                ((-(mu - dat)) *
                  (mu^3 *
                    pi^4 +
                    480 * euler_gamma^3 * mu^2 * (dat - z) +
                    72 * euler_gamma^2 * mu^2 * pi^2 * (dat - z) +
                    540 * euler_gamma^4 * mu * (dat - z)^2 -
                    540 * euler_gamma^4 * dat * (dat - z)^2 +
                    144 * euler_gamma * mu * pi^2 * (dat - z) * z +
                    72 * euler_gamma^2 * pi^2 * dat * (dat - z) * z +
                    480 * euler_gamma^3 * (dat - z) * (2 * dat - z) * z -
                    pi^4 * dat * z^2 +
                    72 * euler_gamma * mu^2 * pi^2 * (-dat + z) +
                    960 * euler_gamma^3 * mu * dat * (-dat + z) +
                    72 * euler_gamma * pi^2 * z^2 * (-dat + z) -
                    72 * euler_gamma^2 * mu * pi^2 * (dat - z) * (dat + z) +
                    mu * pi^4 * z * (2 * dat + z) -
                    mu^2 * pi^4 * (dat + 2 * z) -
                    96 * (mu - z)^3 * zeta3) +
                  12 *
              ...
              2 *
                ...
                ((-euler_gamma) *
                  ...
                  (mu^3 *
                    pi^4 +
                    120 * euler_gamma^3 * mu^2 * (dat - z) +
                    24 * euler_gamma^2 * mu^2 * pi^2 * (dat - z) +
                    108 * euler_gamma^4 * mu * (dat - z)^2 -
                    108 * euler_gamma^4 * dat * (dat - z)^2 +
                    72 * euler_gamma * mu * pi^2 * (dat - z) * z +
                    24 * euler_gamma^2 * pi^2 * dat * (dat - z) * z +
                    120 * euler_gamma^3 * (dat - z) * (2 * dat - z) * z -
                    pi^4 * dat * z^2 +
                    36 * euler_gamma * mu^2 * pi^2 * (-dat + z) +
                    240 * euler_gamma^3 * mu * dat * (-dat + z) +
                    36 * euler_gamma * pi^2 * z^2 * (-dat + z) -
                    24 * euler_gamma^2 * mu * pi^2 * (dat - z) * (dat + z) +
                    mu * pi^4 * z * (2 * dat + z) -
                    mu^2 * pi^4 * (dat + 2 * z) -
                    96 * (mu - z)^3 * zeta3) +
                  12 *
      ...
      if (!xizero) {
        ...
        infomat <- t(Jac) %*%
          gev.infomat(par = c(mu, sigmaq, xi), method = "exp", nobs = nobs) %*%
          Jac
      } else {
        Jac <- rbind(
          c(1, 0, 0),
        ...
        infomat <- t(Jac) %*%
          gev.infomat(par = c(mu, sigmaq, 0), method = "exp", nobs = nobs) %*%
          Jac
      }
    }
    ...
    if (method == "obs") {
      if (!xizero) {
        k11 <- sum(
          ((N^xi * log1q^(-xi) - 1) * (mu - dat) / (mu - z) + 1)^(-1 / xi - 2) *
            ((N^xi *
              log1q^(-xi) -
              1) *
              (mu - dat) /
              (mu - z)^2 -
              (N^xi * log1q^(-xi) - 1) / (mu - z))^2 *
            (1 / xi + 1) /
            xi -
            2 *
              ((N^xi * log1q^(-xi) - 1) * (mu - dat) / (mu - z) + 1)^(-1 /
                xi -
                1) *
              ((N^xi * log1q^(-xi) - 1) *
                (mu - dat) /
            ...
            ((N^xi * log1q^(-xi) - 1) *
              ...
              (N^xi * log1q^(-xi) - 1) / (mu - z))^2 *
              (1 / xi + 1) /
              ((N^xi * log1q^(-xi) - 1) * (mu - dat) / (mu - z) + 1)^2 +
            2 *
              ((N^xi * log1q^(-xi) - 1) *
                (mu - dat) /
                (mu - z)^3 -
          ...
          -((N^xi * log1q^(-xi) - 1) * (mu - dat) / (mu - z) + 1)^(-1 /
            ...
            ((N^xi * log1q^(-xi) - 1) * (mu - dat) / (mu - z) + 1)^(-1 /
              ...
              ((N^xi *
                ...
                (mu - dat) /
                (mu - z)^2 -
                (N^xi * log1q^(-xi) * logN - N^xi * log1q^(-xi) * loglog1q) /
                  (mu - z)) /
              xi -
            ((N^xi * log1q^(-xi) * logN - N^xi * log1q^(-xi) * loglog1q) *
              (mu - dat) /
            ...
            ((N^xi * log1q^(-xi) - 1) *
              ...
              (mu - z)^2 -
              (N^xi * log1q^(-xi) - 1) / (mu - z)) /
              (((N^xi * log1q^(-xi) - 1) * (mu - dat) / (mu - z) + 1) * xi^2)
        )
        k22 <- sum(
          (N^xi * log1q^(-xi) - 1)^2 *
            ((N^xi * log1q^(-xi) - 1) *
          ...
          (N^xi * log1q^(-xi) - 1) *
            ...
            ((N^xi * log1q^(-xi) * logN - N^xi * log1q^(-xi) * loglog1q) *
              ...
              (((N^xi * log1q^(-xi) - 1) *
                ...
                1) *
                (mu - z)) -
              log1p((N^xi * log1q^(-xi) - 1) * (mu - dat) / (mu - z)) / xi^2) /
            ((mu -
              z)^2 *
              xi) -
            (N^xi * log1q^(-xi) * logN - N^xi * log1q^(-xi) * loglog1q) *
        ...
        k33 <- sum(
          ((N^xi * log1q^(-xi) - 1) * (mu - dat) / (mu - z) + 1)^(-1 / xi) *
            ((N^xi * log1q^(-xi) * logN - N^xi * log1q^(-xi) * loglog1q) *
              (mu - dat) /
              (((N^xi * log1q^(-xi) - 1) * (mu - dat) / (mu - z) + 1) *
                (mu - z) *
              ...
              log1p(
                ...
                xi^2)^2 +
            ((N^xi * log1q^(-xi) - 1) * (mu - dat) / (mu - z) + 1)^(-1 / xi) *
              ((N^xi * log1q^(-xi) * logN - N^xi * log1q^(-xi) * loglog1q)^2 *
                (mu - dat)^2 /
                (((N^xi *
                  log1q^(-xi) -
                  1) *
                ...
                (N^xi *
                  log1q^(-xi) *
                  logN^2 -
                  2 * N^xi * log1q^(-xi) * logN * loglog1q +
                  N^xi * log1q^(-xi) * loglog1q^2) *
                  (mu - dat) /
                  (((N^xi * log1q^(-xi) - 1) * (mu - dat) / (mu - z) + 1) *
                    (mu - z) *
                    xi) +
                2 *
                  ...
                  (((N^xi * log1q^(-xi) - 1) * (mu - dat) / (mu - z) + 1) *
                    ...
                    xi^2) -
                2 *
                  log1p((N^xi * log1q^(-xi) - 1) * (mu - dat) / (mu - z)) /
                  xi^3) -
            (N^xi * log1q^(-xi) * logN - N^xi * log1q^(-xi) * loglog1q)^2 *
              (mu - dat)^2 *
              (1 / xi + 1) /
            ...
            ((mu - z)^2 +
              (-dat + z) *
                (-2 *
                  ...
                  N^((-mu + dat) / (mu - z)) *
                    log1q^((mu - dat) / (mu - z)) *
                    (2 * (mu - z) + (dat - z) * (logN - loglog1q))) *
                (logN - loglog1q))
        )
        k12 <- -sum(
          -((1 / (mu - z)^4) *
            ((mu - z) *
              (mu - z - (mu - 2 * dat + z) * (logN - loglog1q)) +
              N^((-mu + dat) / (mu - z)) *
                log1q^((mu - dat) / (mu - z)) *
                ((mu - z) *
                  (mu - 2 * dat + z) +
                  (mu - dat) * (dat - z) * (logN - loglog1q)) *
                (logN - loglog1q)))
        )
        k13 <- -sum(
          (1 / (2 * (mu - z)^4)) *
            (2 *
              ...
              2 *
                ...
                (mu - z + (dat - z) * (logN - loglog1q)) *
                (logN - loglog1q) +
              2 * (mu - dat) * (mu - z) * (dat - z) * (logN - loglog1q)^2 +
              (mu - z) * (mu - 2 * dat + z) * (-dat + z) * (logN - loglog1q)^2 +
              exp(((mu - dat) * (-logN + loglog1q)) / (mu - z)) *
                (mu - z) *
                (mu - 2 * dat + z) *
                (-dat + z) *
                (logN - loglog1q)^2 -
        ...
        k23 <- -sum(
          (1 / (2 * (mu - z)^4)) *
            ((mu - dat) *
              ((mu - z) *
                (-2 * mu + 2 * z + (mu - 2 * dat + z) * (logN - loglog1q)) +
                N^((-mu + dat) / (mu - z)) *
                  log1q^((mu - dat) / (mu - z)) *
                  ((-(mu - z)) *
                    (mu - 2 * dat + z) -
  ...
  if (qty == "quantile") {
    ...
    cbind(
      (mu - z) *
        ...
        (N^xi * gamma(-xi + 1) - 1),
      -(dat - mu) / (mu - z),
      (-(N^xi * gamma(-xi + 1) - 1) * (dat - mu) / (mu - z) + 1)^(-1 / xi) *
        (mu - z) *
        xi *
        ((N^xi *
          log(N) *
...
gevN.phi <- function(par, dat, N, q = 0.5, qty = c("mean", "quantile"), V) {
  ...
  if (qty == "mean") {
    ...
    matrix(
      -(N^xi * log(1 / q)^(-xi) - 1) *
        (-(N^xi * log(1 / q)^(-xi) - 1) * (dat - mu) / (mu - z) + 1)^(-1 /
          xi -
          1) /
        ((mu - z) * xi) -
        (N^xi * log(1 / q)^(-xi) - 1) *
          (1 / xi + 1) /
          (((N^xi * log(1 / q)^(-xi) - 1) * (dat - mu) / (mu - z) - 1) *
            (mu - z)),
      nrow = 1
    ) %*%
      V
  ...
  if (qty == "mean") {
    rbind(
      -(N^xi *
        mu *
        xi^2 *
        ((N^xi * mu * gamma(-xi + 1) - (N^xi * gamma(-xi + 1) - 1) * dat - z) /
          (mu - z))^(1 / xi) *
        gamma(-xi + 1) -
        N^xi *
          xi^2 *
      ...
      (N^xi *
        ...
        N^xi *
          ...
          gamma(-xi + 1) -
        N^xi * mu * xi^2 * log(N) * gamma(-xi + 1) +
        N^xi * xi^2 * z * log(N) * gamma(-xi + 1) +
        N^xi * mu * xi^2 * psigamma(-xi + 1) * gamma(-xi + 1) -
        N^xi *
          xi^2 *
          z *
          psigamma(-xi + 1) *
        ...
        N^(2 * xi) *
          ...
          ((N^xi *
            ...
            (mu - z))^(1 / xi) *
          gamma(-xi + 1)^2 -
        N^(2 * xi) * dat * xi * log(N) * gamma(-xi + 1)^2 +
        N^(2 * xi) * mu * xi * log(N) * gamma(-xi + 1)^2 +
        N^(2 * xi) * dat * xi * psigamma(-xi + 1) * gamma(-xi + 1)^2 -
        N^(2 * xi) * mu * xi * psigamma(-xi + 1) * gamma(-xi + 1)^2 -
        2 *
          N^xi *
          dat *
          xi *
        ...
        N^xi * mu * xi * psigamma(-xi + 1) * gamma(-xi + 1) -
        N^(2 * xi) *
          dat *
          xi *
          gamma(
        ...
        N^(2 * xi) *
          ...
          gamma(-xi + 1)^2 +
        2 * N^xi * dat * xi * gamma(-xi + 1) -
        N^xi * mu * xi * gamma(-xi + 1) -
        N^xi * xi * z * gamma(-xi + 1) -
        2 *
          N^xi *
          dat *
          log(
...
rlarg.ll <- function(par, dat) {
  ...
  if (!isTRUE(all(dat[1, 1] >= dat[1, ]))) {
    stop(
      "Observations in \"dat\" must be ordered from largest to smallest in each row."
    )
  }
  r <- ncol(dat)
  n <- nrow(dat)
  ...
  if (
    (par[3] < 0) &
      (xmax > par[1] - par[2] / par[3]) ||
      (par[3] > 0) & (xmin < par[1] - par[2] / par[3])
  ) {
    return(-Inf)
  }
  if (isTRUE(all.equal(par[3], -1, check.attributes = FALSE))) {
    -sum((1 + par[3] * (dat[, r] - par[1]) / par[2])^(-1 / par[3])) -
      n * r * log(par[2])
  } else {
    if (abs(par[3]) > 1e-7) {
      -sum((1 + par[3] * (dat[, r] - par[1]) / par[2])^(-1 / par[3])) -
...
rlarg.score <- function(par, dat) {
  ...
  if (abs(xi) > 1e-7) {
    score <- cbind(
      ...
      -(-(mu - dat[, r]) * xi / sigma + 1)^(-1 / xi) *
        (log1p(-(mu - dat[, r]) * xi / sigma) /
          xi^2 -
    ...
    score <- c(
      sum(-exp(-dat[, r] / sigma + mu / sigma) / sigma) + length(dat) / sigma,
      -n *
        r /
        sigma +
        sum((mu - dat[, r]) / sigma^2 * exp((mu - dat[, r]) / sigma)) +
        sum((dat - mu) / sigma^2),
      scorexi
    )
    return(score)
...
rlarg.infomat <- function(
  ...
  if (method == "exp") {
    ...
    if (!xizero) {
      m12a <- (-gamma(1 + r + xi) + (r + xi * (2 + xi)) * gamma(r + 2 * xi)) /
        (tau^2 * xi * gamma(r))
      m13a <- ((-r) *
        xi *
        gamma(r + xi) -
        (r + xi * (2 + xi)) * gamma(r + 2 * xi) +
        gamma(1 + r + xi) * (1 + xi + xi * digamma(1 + r + xi))) /
        (tau * xi^2 * gamma(r))
      m22a <- -((gamma(1 + r) -
        2 * gamma(1 + r + xi) +
        (r + xi * (2 + xi)) * gamma(r + 2 * xi)) /
        (tau^2 * xi^2 * gamma(r)))
      m23a <- (1 / (tau * xi^3 * gamma(r))) *
        ((r + xi * (2 + xi)) *
          gamma(r + 2 * xi) +
          gamma(1 + r) * (1 + xi * digamma(1 + r)) -
          gamma(r + xi) *
            (2 * r + xi * (2 + xi) + xi * (r + xi) * digamma(1 + r + xi)))
    } else {
      m12a <- (1 + r * digamma(r)) / tau^2
      m13a <- -((digamma(r) *
        (2 + r * digamma(r)) +
        r * psigamma(deriv = 1, r)) /
        (2 * tau))
      m22a <- -((1 +
        digamma(r) * (2 + r * digamma(r)) +
        r * psigamma(deriv = 1, r)) /
        tau^2)
      m23a <- (1 / (2 * tau)) *
        (3 *
          ...
          digamma(r) *
            (2 +
              digamma(r) * (3 + r * digamma(r)) +
              3 * r * psigamma(deriv = 1, r)) +
          r * psigamma(deriv = 2, r))
    }
    if (abs(xi) > 1e-3) {
      m33a <- -((1 / (xi^4 * gamma(r))) *
        ((r + xi * (2 + xi)) *
          ...
          2 *
            gamma(r + xi) *
            (r + xi + xi^2 + xi * (r + xi) * digamma(1 + r + xi)) +
          gamma(r) *
            (r +
              xi * (2 + xi) +
              r *
                ...
                (2 *
                  digamma(r) +
                  xi * (digamma(1 + r)^2 + psigamma(deriv = 1, 1 + r))))))
    } else {
      m33a <- -(4 *
        psigamma(deriv = 0, r)^3 +
        r * psigamma(deriv = 0, r)^4 +
        psigamma(deriv = 1, r) * (4 + 3 * r * psigamma(deriv = 1, r)) +
        psigamma(deriv = 0, r)^2 *
          (4 + 6 * r * psigamma(deriv = 1, r)) +
        4 * psigamma(deriv = 2, r) +
        4 *
          psigamma(deriv = 0, r) *
          (3 * psigamma(deriv = 1, r) + r * psigamma(deriv = 2, r)) +
        r * psigamma(deriv = 3, r)) /
        4
    }
    ma <- matrix(
      c(m11a, m12a, m13a, m12a, m22a, m23a, m13a, m23a, m33a),
      nrow = 3,
      ncol = 3
    )
    if (r == 1L) {
      ...
      if (!xizero) {
        m11b <- -((xi^2 * gamma(r + 2 * xi)) /
          (tau^2 * (1 + 2 * xi) * gamma(r)))
        m12b <- (xi * gamma(r + 2 * xi)) / (tau^2 * (1 + 2 * xi) * gamma(r))
        m13b <- (gamma(r + xi) / (1 + xi) - gamma(r + 2 * xi) / (1 + 2 * xi)) /
          (tau * gamma(r))
        m22b <- -(gamma(r + 2 * xi) / (tau^2 * (1 + 2 * xi) * gamma(r)))
        m23b <- (-(gamma(r + xi) / (1 + xi)) +
          gamma(r + 2 * xi) / (1 + 2 * xi)) /
          (tau * xi * gamma(r))
        m33b <- (-1 +
          (2 * gamma(r + xi)) / (gamma(r) + xi * gamma(r)) -
          gamma(r + 2 * xi) / (gamma(r) + 2 * xi * gamma(r))) /
          xi^2
      } else {
        m11b <- 0
        m12b <- 0
      ...
      mb <- matrix(
        c(m11b, m12b, m13b, m12b, m22b, m23b, m13b, m23b, m33b),
        nrow = 3,
        ncol = 3
      )
    ...
    if (!xizero) {
      d11a <- sum(
        xi^2 *
          (r / xi + 1) /
          (tau^2 * ((eta - yr) * xi / tau - 1)^2) +
          (-(eta - yr) * xi / tau + 1)^(1 / xi - 2) *
            xi *
            (1 / xi - 1) /
            (tau^2 * (-(eta - yr) * xi / tau + 1)^(2 / xi)) -
          2 *
            (-(eta - yr) * xi / tau + 1)^(2 / xi - 2) /
            (tau^2 * (-(eta - yr) * xi / tau + 1)^(3 / xi))
      )
      d12a <- sum(
        xi *
          (r / xi + 1) /
          (tau^2 * ((eta - yr) * xi / tau - 1)) -
          (eta - yr) *
            xi^2 *
            (r / xi + 1) /
            (tau^3 * ((eta - yr) * xi / tau - 1)^2) -
          (eta - yr) *
            (-(eta - yr) * xi / tau + 1)^(1 / xi - 2) *
            xi *
            (1 / xi - 1) /
            (tau^3 * (-(eta - yr) * xi / tau + 1)^(2 / xi)) +
          (-(eta - yr) * xi / tau + 1)^(1 / xi - 1) /
            (tau^2 * (-(eta - yr) * xi / tau + 1)^(2 / xi)) +
          2 *
            (eta - yr) *
            (-(eta - yr) * xi / tau + 1)^(2 / xi - 2) /
            (tau^3 * (-(eta - yr) * xi / tau + 1)^(3 / xi))
      )
      d13a <- sum(
        -(r / xi + 1) /
          (tau * ((eta - yr) * xi / tau - 1)) +
          ...
          (-(eta - yr) * xi / tau + 1)^(1 / xi - 1) *
            ((eta - yr) *
              (1 / xi - 1) /
              (tau * ((eta - yr) * xi / tau - 1)) -
              log(-(eta - yr) * xi / tau + 1) / xi^2) /
            (tau * (-(eta - yr) * xi / tau + 1)^(2 / xi)) -
          2 *
            (-(eta - yr) * xi / tau + 1)^(1 / xi - 1) *
            (log(-(eta - yr) * xi / tau + 1) /
              xi^2 -
              (eta - yr) / (tau * ((eta - yr) * xi / tau - 1) * xi)) /
            (tau * (-(eta - yr) * xi / tau + 1)^(2 / xi)) +
          r / (tau * ((eta - yr) * xi / tau - 1) * xi)
      )
      d22a <- sum(
        -2 *
          (eta - yr) *
          xi *
          (r / xi + 1) /
          (tau^3 * ((eta - yr) * xi / tau - 1)) +
          (eta - yr)^2 *
            xi^2 *
            (r / xi + 1) /
            (tau^4 * ((eta - yr) * xi / tau - 1)^2) +
          (eta - yr)^2 *
            (-(eta - yr) * xi / tau + 1)^(1 / xi - 2) *
            xi *
            (1 / xi - 1) /
            (tau^4 * (-(eta - yr) * xi / tau + 1)^(2 / xi)) +
          1 / tau^2 -
          2 *
            (eta - yr) *
            (-(eta - yr) * xi / tau + 1)^(1 / xi - 1) /
        ...
        (eta - yr) *
          ...
          (eta - yr) *
            ...
            (log(-(eta - yr) * xi / tau + 1) /
              ...
              (eta - yr) / (tau * ((eta - yr) * xi / tau - 1) * xi)) /
            (tau^2 * (-(eta - yr) * xi / tau + 1)^(2 / xi)) -
          (eta - yr) * r / (tau^2 * ((eta - yr) * xi / tau - 1) * xi) +
          (eta - yr)^2 /
            (tau^3 *
              (-(eta - yr) * xi / tau + 1)^(1 / xi) *
              ((eta - yr) * xi / tau - 1)^2)
      )
      d33a <- sum(
        -(log(-(eta - yr) * xi / tau + 1) /
          xi^2 -
          (eta - yr) / (tau * ((eta - yr) * xi / tau - 1) * xi))^2 /
          (-(eta - yr) * xi / tau + 1)^(1 / xi) +
          (2 *
            log(-(eta - yr) * xi / tau + 1) /
            xi^3 -
            2 * (eta - yr) / (tau * ((eta - yr) * xi / tau - 1) * xi^2) -
            (eta - yr)^2 / (tau^2 * ((eta - yr) * xi / tau - 1)^2 * xi)) /
            (-(eta - yr) * xi / tau + 1)^(1 / xi) +
          (eta - yr)^2 *
            (r / xi + 1) /
            (tau^2 * ((eta - yr) * xi / tau - 1)^2) -
          2 * r * log(-(eta - yr) * xi / tau + 1) / xi^3 +
          2 * (eta - yr) * r / (tau * ((eta - yr) * xi / tau - 1) * xi^2)
      )
      da <- -matrix(
        c(d11a, d12a, d13a, d12a, d22a, d23a, d13a, d23a, d33a),
        nrow = 3,
        ncol = 3
      )
    } else {
      ...
      d130a <- sum(
        1 /
          ...
          (2 *
            ...
            exp(eta / tau) -
            yr^2 * exp(eta / tau) -
            (eta^2 + 2 * eta * tau) * exp(eta / tau) +
            2 * (eta * r * tau - r * tau * yr + tau^2) * exp(yr / tau)) *
          exp(-yr / tau) /
          tau^3
      )
      d220a <- sum(
        (2 *
          ...
          yr^2 * exp(eta / tau) -
          (eta^2 + 2 * eta * tau) * exp(eta / tau) +
          (2 * eta * r * tau - 2 * r * tau * yr + tau^2) * exp(yr / tau)) *
          exp(-yr / tau) /
          tau^4
      )
      d230a <- sum(
        1 /
          ...
          ((3 * eta + 2 * tau) *
            ...
            exp(eta / tau) -
            yr^3 * exp(eta / tau) -
            (3 * eta^2 + 4 * eta * tau) * yr * exp(eta / tau) +
            (eta^3 + 2 * eta^2 * tau) * exp(eta / tau) -
            2 *
              (eta^2 *
                r *
                tau +
      ...
      d330a <- sum(
        1 /
          ...
          (4 *
            ...
            exp(eta / tau) -
            3 * yr^4 * exp(eta / tau) -
            6 * (3 * eta^2 + 4 * eta * tau) * yr^2 * exp(eta / tau) +
            12 * (eta^3 + 2 * eta^2 * tau) * yr * exp(eta / tau) -
            (3 * eta^4 + 8 * eta^3 * tau) * exp(eta / tau) +
            4 *
              (2 *
                eta^3 *
                r *
                ...
                2 * r * tau * yr^3 +
                3 * eta^2 * tau^2 +
                3 * (2 * eta * r * tau + tau^2) * yr^2 -
                6 * (eta^2 * r * tau + eta * tau^2) * yr) *
              exp(yr / tau)) *
          exp(-yr / tau) /
          tau^4
      )
      da <- -matrix(
        c(d110a, d120a, d130a, d120a, d220a, d230a, d130a, d230a, d330a),
        nrow = 3,
        ncol = 3
      )
    }
    if (r == 1L) {
      ...
      if (!xizero) {
        d11b <- sum(
          -2 *
            ...
            (1 / xi + 1) /
            (((eta - yr) * xi - tau)^3 *
              (xi * (y - yr) / ((eta - yr) * xi - tau) - 1)) +
            xi^4 *
              (y - yr)^2 *
              (1 / xi + 1) /
              (((eta - yr) * xi - tau)^4 *
                (xi * (y - yr) / ((eta - yr) * xi - tau) - 1)^2) +
            xi^2 / ((eta - yr) * xi - tau)^2
        )
        d12b <- sum(
          2 *
            xi^2 *
          ...
          ((eta - yr) *
            ...
            2 *
              ((eta - yr)^2 *
                ...
                (y - yr) /
                ((eta - yr) * xi - tau)^3 -
                (eta - yr) * (y - yr) / ((eta - yr) * xi - tau)^2) *
              (1 / xi + 1) /
              (xi * (y - yr) / ((eta - yr) * xi - tau) - 1) +
            (eta - yr)^2 / ((eta - yr) * xi - tau)^2 -
            2 *
              ((eta - yr) *
                xi *
                (y - yr) /
                ((eta - yr) * xi - tau)^2 -
                (y - yr) / ((eta - yr) * xi - tau)) /
              (xi^2 * (xi * (y - yr) / ((eta - yr) * xi - tau) - 1)) -
            2 * log(-xi * (y - yr) / ((eta - yr) * xi - tau) + 1) / xi^3
        )
      } else {
        d11b <- 0
        d12b <- 0
        ...
        d23b <- sum(
          -(eta * tau - (2 * eta + tau) * y + y^2 + 2 * eta * yr - yr^2) / tau^3
        )
        d33b <- sum(
          1 /
            3 *
      ...
      db <- -matrix(
        c(d11b, d12b, d13b, d12b, d22b, d23b, d13b, d23b, d33b),
        nrow = 3,
        ncol = 3
      )
      infomat <- da + db
...
pp.ll <- function(par, dat, u, np = 1) {
  ...
  if (abs(par[3]) > 1e-6) {
    -np[1] *
      (1 + par[3] * (u[1] - par[1]) / par[2])^(-1 / par[3]) -
      length(dat) * log(par[2]) -
      (1 + 1 / par[3]) * sum(log1p(par[3] * (dat - par[1]) / par[2]))
  } else {
    -np[1] *
      exp((par[1] - u) / par[2]) -
      length(dat) * log(par[2]) +
...
pp.score <- function(par, dat, u, np = 1) {
  ...
  if (!isTRUE(all.equal(xi, 0, check.attributes = FALSE))) {
    c(
      -sum(xi * (1 / xi + 1) / (sigma * ((mu - dat) * xi / sigma - 1))) -
        np * (-(mu - u) * xi / sigma + 1)^(-1 / xi - 1) / sigma,
      sum(
        (mu - dat) *
          xi *
          (1 / xi + 1) /
        ...
        (mu - u) *
          np *
          ((u - mu) * xi / sigma + 1)^(1 / xi - 1) /
          (sigma^2 * ((u - mu) * xi / sigma + 1)^(2 / xi)),
      -sum(
        (mu - dat) * (1 / xi + 1) / (sigma * ((mu - dat) * xi / sigma - 1))
      ) +
        sum(log1p(-(mu - dat) * xi / sigma) / xi^2) -
        np *
          (log1p((u - mu) * xi / sigma) /
            xi^2 -
            (mu - u) / (sigma * ((mu - u) * xi / sigma - 1) * xi)) /
          (-(mu - u) * xi / sigma + 1)^(1 / xi)
    )
  } else {
    c(
      -np * exp((mu - u) / sigma) / sigma + nu / sigma,
      np *
        (mu - u) *
        exp((mu - u) / sigma) /
        sigma^2 -
      ...
      -1 /
        ...
        exp((mu - u) / sigma) /
        sigma^2 +
        sum(1 / 2 * (mu + 2 * sigma - dat) * (mu - dat) / sigma^2)
    )
  }
}
...
pp.infomat <- function(
  ...
  if (method == "obs") {
    zm <- (dat - mu) / sigma
    if (!xizero) {
      d11 <- -sum(xi^2 * (1 / xi + 1) / ((xi * zm + 1)^2 * sigma^2)) -
        (vm * xi + 1)^(1 / xi - 2) *
          m *
          xi *
          (1 / xi - 1) /
          ((vm * xi + 1)^(2 / xi) * sigma^2) +
        2 * (vm * xi + 1)^(2 / xi - 2) * m / ((vm * xi + 1)^(3 / xi) * sigma^2)
      d12 <- (vm * xi + 1)^(-1 / xi - 2) *
        m *
        vm *
        xi *
        (1 / xi + 1) /
        sigma^2 -
        xi^2 * sum(zm * (1 / xi + 1) / ((xi * zm + 1)^2 * sigma^2)) -
        (vm * xi + 1)^(-1 / xi - 1) * m / sigma^2 +
        xi * (1 / xi + 1) * sum(1 / (xi * zm + 1)) / sigma^2
      d13 <- -(vm * xi + 1)^(-1 / xi - 1) *
        m *
        (vm / ((vm * xi + 1) * xi) - log(vm * xi + 1) / xi^2) /
        sigma +
        sum(xi * zm * (1 / xi + 1) / ((xi * zm + 1)^2 * sigma)) -
        (vm * xi + 1)^(-1 / xi) * m * vm / ((vm * xi + 1)^2 * sigma) -
        sum((1 / xi + 1) / ((xi * zm + 1) * sigma)) +
        sum(1 / ((xi * zm + 1) * sigma * xi))
      d22 <- (vm * xi + 1)^(-1 / xi - 2) *
        m *
        vm^2 *
        xi *
        (1 / xi + 1) /
        sigma^2 -
        sum(xi^2 * zm^2 * (1 / xi + 1) / ((xi * zm + 1)^2 * sigma^2)) -
        2 * (vm * xi + 1)^(-1 / xi - 1) * m * vm / sigma^2 +
        2 * xi * sum(zm * (1 / xi + 1) / ((xi * zm + 1) * sigma^2)) -
        r1 / sigma^2
      d23 <- -(vm * xi + 1)^(-1 / xi - 1) *
        m *
        vm *
        (vm / ((vm * xi + 1) * xi) - log(vm * xi + 1) / xi^2) /
        sigma +
        sum(xi * zm^2 * (1 / xi + 1) / ((xi * zm + 1)^2 * sigma)) -
        (vm * xi + 1)^(-1 / xi) * m * vm^2 / ((vm * xi + 1)^2 * sigma) -
        sum(zm * (1 / xi + 1) / ((xi * zm + 1) * sigma)) +
        sum(zm / ((xi * zm + 1) * sigma * xi))
      d33 <- (vm * xi + 1)^(-1 / xi) *
        m *
        (vm / ((vm * xi + 1) * xi) - log(vm * xi + 1) / xi^2)^2 +
        (vm * xi + 1)^(-1 / xi) *
          m *
          (vm^2 /
            ((vm * xi + 1)^2 * xi) +
            2 * vm / ((vm * xi + 1) * xi^2) -
            2 * log(vm * xi + 1) / xi^3) -
        sum(zm^2 * (1 / xi + 1) / (xi * zm + 1)^2) -
        2 * sum(zm / ((xi * zm + 1) * xi^2)) +
        2 * sum(log(xi * zm + 1)) / xi^3
      infomat <- matrix(
        c(d11, d12, d13, d12, d22, d23, d13, d23, d33),
        nrow = 3,
        ncol = 3,
        byrow = TRUE
      )
    } else {
      #Observed information, xi = 0
      ...
      d22 <- m *
        ...
        exp(-vm) /
        sigma^2 -
        2 * m * vm * exp(-vm) / sigma^2 +
        2 * sum(zm) / sigma^2 -
        r1 / sigma^2
      d13 <- 0.5 *
        (m * vm^2 + 2 * sum(zm) * exp(vm) - 2 * m * vm - 2 * r1 * exp(vm)) *
        exp(-vm) /
        sigma
      d23 <- 0.5 *
        (m *
        ...
        sigma
      d33 <- ((1 / 12) *
        (m * vm^3 * (-8 + 3 * vm) + 4 * exp(vm) * sum(zm^2 * (-3 + 2 * zm)))) /
        exp(vm)
      infomat <- matrix(
        c(d11, d12, d13, d12, d22, d23, d13, d23, d33),
        nrow = 3,
    ...
    if (!xizero) {
      ...
      e13 <- (vm * xi + 1)^(-1 / xi - 1) *
        m *
        (vm * (1 / xi + 1) / (vm * xi + 1) - log(vm * xi + 1) / xi^2) /
        sigma -
        (2 * vm * xi + vm - xi) *
          (vm * xi + 1)^(-1 / xi) *
          m *
          r2 /
          ((vm * xi + 1)^2 * (2 * xi^2 + 3 * xi + 1) * sigma)
      e22 <- -(vm * xi + 1)^(-1 / xi - 2) *
        m *
        vm^2 *
        xi *
        ...
        ((vm * xi + 1)^2 *
          r1 *
          (2 * xi + 1) -
          ((vm * xi + 2) * vm * (2 * xi + 1) + 2) * r2 * (xi + 1)) *
          (vm * xi + 1)^(-1 / xi) *
          m /
          ((sigma * vm * xi + sigma)^2 * (2 * xi + 1))
      e23 <- (vm * xi + 1)^(-1 / xi - 1) *
        m *
        vm *
        (vm * (1 / xi + 1) / (vm * xi + 1) - log(vm * xi + 1) / xi^2) /
        sigma -
        ((vm * xi + vm + 1) * vm * (2 * xi + 1) + 1) *
          (vm * xi + 1)^(-1 / xi) *
          m *
          r2 /
          ((vm * xi + 1)^2 * sigma * (2 * xi + 1) * (xi + 1))
      e33 <- -(vm * xi + 1)^(-1 / xi) *
        m *
        (vm / ((vm * xi + 1) * xi) - log(vm * xi + 1) / xi^2)^2 -
        (vm * xi + 1)^(-1 / xi) *
        ...
        (2 *
          ...
          (vm^2 *
            ...
            (xi - 3) *
            xi +
            2 * ((2 * xi^2 - xi - 3) * xi - 1) * vm +
            2 * xi^2) *
            xi) *
          (vm * xi + 1)^(-1 / xi) *
          m *
          r2 /
          ((vm * xi + 1)^2 * (2 * xi + 1) * (xi + 1) * xi^3)
      infomat <- -nobs *
        matrix(
          c(e11, e12, e13, e12, e22, e23, e13, e23, e33),
          nrow = 3,
          ncol = 3,
          byrow = TRUE
        )
      ...
      e22 <- -2 *
        ...
        exp(-vm) /
        sigma^2 -
        m * vm^2 * exp(-vm) / sigma^2 +
        m * r1 * exp(-vm) / sigma^2 -
        2 * m * r2 * exp(-vm) / sigma^2 +
        2 * m * vm * exp(-vm) / sigma^2
      e23 <- -1 /
        2 *
        (2 * r2 * vm^2 + vm^3 + 2 * r2 * vm - 2 * vm^2 + 2 * r2) *
        m *
        exp(-vm) /
        sigma
      e33 <- -1 /
