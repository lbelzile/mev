...
gev.bias <- function(par, n) {
  ...
  if (abs(xi) > 1e-10) {
    k112 <- (xi + 1) *
      (gamma(2 * xi + 2) - (xi + 1) * (4 * xi + 1) * gamma(3 * xi + 1)) /
      (sigma^3 * xi)
    k12.2 <- 2 *
      ((xi + 1)^2 * gamma(2 * xi + 1) - gamma(xi + 2)) /
      (sigma^3 * xi)
  } else {
  ...
  if (abs(xi) > 1e-06) {
    ...
    k12.3 <- -(2 *
      ...
      psigamma(xi + 2) *
        gamma(xi + 2)) /
      (sigma^2 * xi) +
      ((xi + 1)^2 * gamma(2 * xi + 1) - gamma(xi + 2)) / (sigma^2 * xi^2)
    k13.2 <- -(((xi + 1)^2 *
      gamma(2 * xi + 1) -
      xi * ((xi + 1) / xi + psigamma(xi + 1)) * gamma(xi + 2)) /
      (sigma^2 * xi^2))
    k22.2 <- -2 *
      ((xi + 1)^2 * gamma(2 * xi + 1) - 2 * gamma(xi + 2) + 1) /
      (sigma^3 * xi^2)
  } else {
    # Compute the approximation for xi=0, (limit)
    k113_0 <- -1 /
      12 *
      (36 * euler_gamma - 6 * euler_gamma^2 - pi^2 - 24) /
      sigma^2
    k122_0 <- -1 /
      6 *
      (36 * euler_gamma - 6 * euler_gamma^2 - pi^2 - 24) /
      sigma^3
    k12.3_0 <- (6 * euler_gamma - 3 * euler_gamma^2 - 1 / 2 * pi^2 - 2) /
      (2 * sigma^2)
    k13.2_0 <- 1 / 12 * (12 * euler_gamma - 6 * euler_gamma^2 - pi^2) / sigma^2
    k22.2_0 <- 1 /
      3 *
      (12 * euler_gamma - 6 * euler_gamma^2 - pi^2 - 6) /
      sigma^3
  ...
  if (abs(xi) > 1e-04) {
    ...
    k13.3 <- -(-(2 *
      ...
      gamma(2 * xi + 1) -
      xi *
        ((xi + 1) / xi + psigamma(xi + 1)) *
        psigamma(
          xi +
            2
        ) *
        gamma(xi + 2) +
      xi * ((xi + 1) / xi^2 - 1 / xi - psigamma(xi + 1, 1)) * gamma(xi + 2) +
      2 * (xi + 1) * gamma(2 * xi + 1) -
      ((xi + 1) / xi + psigamma(xi + 1)) * gamma(xi + 2)) /
      (sigma * xi^2) +
      2 *
        ((xi + 1)^2 *
          gamma(2 * xi + 1) -
    ...
    k13.3_0 <- (-6 *
      euler_gamma -
      4 * euler_gamma^3 -
      7 / 2 * euler_gamma * pi^2 +
      3 / 2 * pi^2 * (euler_gamma - 1) +
      12 * euler_gamma^2 +
      7 / 2 * pi^2 -
      8 * zeta3) /
      (6 * sigma)
      ...
      k13.3_l <- sapply(xit, function(xi) {
        -(-(2 *
          ...
          xi *
            ...
            gamma(xi + 2) +
          xi *
            ((xi + 1) / xi^2 - 1 / xi - psigamma(xi + 1, 1)) *
            gamma(xi + 2) +
          2 *
            (xi + 1) *
            gamma(
  ...
  if (abs(xi) > 0.001) {
    k23.3 <- -((2 *
      ...
      ((xi + 1) / xi + psigamma(xi + 1)) *
        ...
        gamma(xi + 2) +
      ((xi + 1) / xi^2 - 1 / xi - psigamma(xi + 1, 1)) * gamma(xi + 2) -
      (xi + 1)^2 * gamma(2 * xi + 1) / xi^2 +
      2 * (xi + 1) * gamma(2 * xi + 1) / xi -
      psigamma(xi + 2) * gamma(xi + 2) / xi +
      (gamma(xi + 2) - 1) / xi^2) /
      (sigma * xi^2) -
      2 *
        ((digamma(1)) +
    ...
    k223 <- -(1 +
      ...
      5 * xi * gamma(2 + xi) +
      3 * (1 + xi) * gamma(1 + 2 * xi) +
      10 * xi * (1 + xi) * gamma(1 + 2 * xi) +
      4 * xi^2 * (1 + xi) * gamma(1 + 2 * xi) -
      gamma(1 + 3 * xi) -
      6 * xi * gamma(1 + 3 * xi) -
      2 *
        xi *
    ...
    k233 <- (1 +
      ...
      gamma(1 + xi) *
        (3 +
          ...
          13 * xi^2 +
          4 * xi^3 +
          2 * xi * (2 + 5 * xi + 3 * xi^2) * psigamma(2 + xi, 0) +
          xi^2 * (1 + xi) * psigamma(2 + xi, 0)^2 +
          xi^2 * (1 + xi) * psigamma(2 + xi, 1))) /
      (sigma * xi^5)
  } else {
    ...
    k233_0 <- 1 /
      ...
      (12 *
        ...
        140 * euler_gamma^4 -
        21 * pi^4 +
        20 * euler_gamma^3 * (pi^2 + 16) -
        4 * euler_gamma^2 * (35 * pi^2 - 60 * zeta3 + 48) +
        8 * pi^2 * (5 * zeta3 - 4) +
        euler_gamma * (9 * pi^4 + 160 * pi^2 - 1120 * zeta3) +
        288 *
          zeta5 +
        640 * zeta3) /
      sigma
        ...
        ((1 + xi)^2 *
          ...
          (1 + 2 * xi) *
            ...
            (1 +
              ...
              5 * xi^2 +
              2 * xi^3 +
              2 * xi * (1 + 3 * xi + 2 * xi^2) * psigamma(2 + xi, 0) +
              xi^2 * (1 + xi) * psigamma(2 + xi, 0)^2 +
              (xi^2) *
                (1 + xi) *
                psigamma(
  ...
  if (abs(xi) > 0.01) {
    k333 <- (-3 *
      gamma(3 + 2 * xi) *
      (1 + 6 * xi + 4 * xi^2 + xi * (1 + 2 * xi) * psigamma(2 + 2 * xi, 0)) +
      6 *
        (1 + 2 * xi) *
        gamma(1 + xi) *
        (1 +
          ...
          7 * xi^2 +
          4 * xi^3 +
          2 * xi * (1 + 4 * xi + 3 * xi^2) * psigamma(2 + xi, 0) +
          xi^2 *
            (1 +
              xi) *
            psigamma(2 + xi, 0)^2 +
      ...
      (1 + 2 * xi) *
        (-2 -
          6 * (4 + digamma(1)) * xi -
          (30 + 42 * digamma(1) + 6 * digamma(1)^2 + pi^2) * xi^2 +
          2 * (1 + xi)^2 * (1 + 4 * xi) * gamma(1 + 3 * xi) +
          xi^3 *
            (-8 -
              18 * digamma(1)^2 -
              2 * digamma(1)^3 -
    ...
    k33.3 <- 2 *
      ((xi + 1)^2 *
        ...
        ((xi + 1) / xi + psigamma(xi + 1)) *
          ...
          gamma(xi + 2) /
          xi +
        ((xi + 1) / xi^2 - 1 / xi - psigamma(xi + 1, 1)) * gamma(xi + 2) / xi -
        (xi + 1)^2 * gamma(2 * xi + 1) / xi^3 +
        (xi + 1) * gamma(2 * xi + 1) / xi^2 +
        ((xi + 1) / xi + psigamma(xi + 1)) * gamma(xi + 2) / xi^2 -
        (digamma(1) + 1 / xi + 1) / xi^2) /
      xi^2 -
      1 /
        3 *
        (pi^2 +
          6 * (digamma(1) + 1 / xi + 1)^2 +
          6 * (xi + 1)^2 * gamma(2 * xi + 1) / xi^2 -
          12 * ((xi + 1) / xi + psigamma(xi + 1)) * gamma(xi + 2) / xi) /
        xi^3
  } else {
  ...
  A1 <- 0.5 *
    cbind(c(k111, k112, k113), c(k112, k122, k123), c(k113, k123, k133))
  A2 <- -cbind(
    c(k11.2, k12.2, k13.2),
    c(k12.2, k22.2, k23.2),
    c(k13.2, k23.2, k33.2)
  ) +
    0.5 * cbind(c(k112, k122, k123), c(k122, k222, k223), c(k123, k223, k233))
  A3 <- -cbind(
    c(k11.3, k12.3, k13.3),
    c(k12.3, k22.3, k23.3),
    c(k13.3, k23.3, k33.3)
  ) +
    0.5 * cbind(c(k113, k123, k133), c(k123, k223, k233), c(k133, k233, k333))
  # Information matrix
  infomat <- gev.infomat(
    par = c(0, sigma, xi),
    dat = 1,
...
gpd.bias <- function(par, n) {
  ...
  c(par[1] * (3 + 5 * par[2] + 4 * par[2]^2), -(1 + par[2]) * (3 + par[2])) /
    (n + 3 * n * par[2])
}
...
gpd.Fscore <- function(par, dat, method = c("obs", "exp")) {
  ...
  gpd.score(par, dat) -
    gpd.infomat(par, dat, method) %*% gpd.bias(par, length(dat))
}
...
gev.Fscore <- function(par, dat, method = "obs") {
  ...
  if (((mdat - par[1]) * par[3] + par[2]) < 0) {
    ...
    gev.score(par, dat) -
      gev.infomat(par, dat, method) %*% gev.bias(par, length(dat))
  }
}
...
gpd.bcor <- function(
  ...
  bcor <- function(par, dat) {
    ...
    if (st[2] < -1 / 3 || st[2] > 1) {
      ...
      warning(
        "Error in \"gpd.bcor\": invalid starting value for the shape (less than -1/3). Aborting"
      )
      return(rep(NA, 2))
    }
    # Root finding
    ...
    if (!inherits(bcor.rf, what = "try-error")) {
      if (
        bcor.rf$termcd == 1 ||
          (bcor.rf$termcd == 2 &&
            isTRUE(all.equal(bcor.rf$fvec, rep(0, 2), tolerance = 1e-06)))
      ) {
        return(bcor.rf$x)
      }
    }
  ...
  bcorF <- function(par, dat, method = c("obs", "exp")) {
    ...
    start <- st.opt <- par
    if (isTRUE(st.opt[2] < -0.25)) {
      start <- mev::fit.gpd(xdat = dat, fpar = list(shape = -0.25))$par
    }
    bcor.st <- try(
      alabama::constrOptim.nl(
        par = start,
        fn = function(pars, dat, met, mdat, ...) {
          sum(gpd.Fscore(par = pars, dat = dat, method = met)^2)
        },
        hin = function(pars, dat, met, mdat, ...) {
          c(
            pars[1],
            pars[2] + 1 / 3,
            pars[2] * mdat + pars[1]
    ...
    if (isTRUE(try(bcor.st$convergence == 0))) {
      st <- bcor.st$par
    } else {
      st <- st.opt
    }
    ...
    par.firth <- try(
      suppressWarnings(nleqslv::nleqslv(
        fn = gpd.Fscoren,
        x = st,
        dat = dat,
        met = method,
    ...
    if (!inherits(par.firth, what = "try-error")) {
      if (
        par.firth$termcd == 1 ||
          (par.firth$termcd == 2 &&
            isTRUE(all.equal(par.firth$fvec, rep(0, 2), tolerance = 1e-06)))
      ) {
        return(par.firth$x)
      }
    }
...
gev.bcor <- function(
  ...
  bcor <- function(par, dat) {
    ...
    if (st[3] < -1 / 3 || st[3] > 1) {
      # If value is the MLE, make sure that it is a sensible starting value i.e. does not violate the constraints for the moments.
      warning(
        "Error in \"gev.bcor\": invalid starting value for the shape (less than -1/3). Aborting"
      )
      return(rep(NA, 3))
    }
    # Root finding
    ...
    bcor.rf <- try(
      nleqslv::nleqslv(
        ...
        par = par,
        dat = dat,
        control = list(maxit = 1000, xtol = 1e-10)
      ),
      silent = TRUE
    )
    if (!inherits(bcor.rf, what = "try-error")) {
      if (
        bcor.rf$termcd == 1 ||
          (bcor.rf$termcd %in%
            c(2, 3) &&
            isTRUE(all.equal(bcor.rf$fvec, rep(0, 3), tolerance = 1e-06)))
      ) {
        return(bcor.rf$x)
      }
      bcor.rf <- try(
      ...
      if (!inherits(bcor.rf, what = "try-error")) {
        if (
          ...
          abs(bcor.rf$x[3]) < 0.025 &&
            bcor.rf$termcd == 2 &&
            isTRUE(all.equal(bcor.rf$fvec, rep(0, 3), tolerance = 0.01))
        ) {
          warning(paste0(
            "Approximate solution for implicit bias correction - the shape is close to zero"
          ))
          return(bcor.rf$x)
        }
      }
    ...
    if (!inherits(par.firth, what = "try-error")) {
      ...
      if (
        par.firth$termcd == 1 ||
          (par.firth$termcd %in%
            c(2, 3) &&
            isTRUE(all.equal(par.firth$fvec, rep(0, 3), tolerance = 1e-06)))
      ) {
        return(par.firth$x)
      }
      ...
      if (!inherits(par.firth, what = "try-error")) {
        if (
          ...
          abs(par.firth$x[3]) < 0.025 &&
            par.firth$termcd == 2 &&
            isTRUE(all.equal(par.firth$fvec, rep(0, 3), tolerance = 0.05))
        ) {
          warning(paste0(
            "Approximate solution for Firth's score equation with method \"",
            method,
            "\" - the shape is close to zero"
          ))
          return(par.firth$x)
        }
      }
...
#' @export
#' @keywords internal
.gev.postpred <- function(
  x,
  posterior,
  Nyr = 100,
  type = c("density", "quantile")
) {
  rowMeans(cbind(apply(rbind(posterior), 1, function(par) {
    switch(
      ...
      density = mev::dgev(
        x = x,
        loc = par[1] - par[2] * (1 - Nyr^par[3]) / par[3],
        scale = par[2] * Nyr^par[3],
        shape = par[3]
      ),
      quantile = mev::qgev(
        p = x,
...
gev.Nyr <- function(
  ...
  estimate <- as.numeric(switch(
    ...
    median = ifelse(
      xi == 0,
      mu + sigma * (log(N) - log(log(2))),
      mu - sigma / xi * (1 - (N / log(2))^xi)
    ),
    mean = ifelse(
      xi == 0,
      mu +
  ...
  if (type == "retlev") {
    ...
    if (p == 0) {
      if (xi < 0) {
        ...
        stop(
          "Invalid argument; maximum likelihood estimate of the uptyper endpoint is Inf"
        )
      }
    }
    # Variance estimate based on delta-method
    var_retlev <- t(grad_retlev) %*%
      solve(gev.infomat(par = par, dat = 1, method = "exp", nobs = nobs)) %*%
      grad_retlev
  } else if (type == "median") {
    ...
    if (xi == 0) {
      ...
      grad_Nmed <- c(
        ...
        sigma *
          ...
          log(N / log(2)) /
          xi -
          sigma * ((N / log(2))^xi - 1) / xi^2
      )
    }
    # Delta-method covariance matrix
    var_Nmed <- t(grad_Nmed) %*%
      solve(gev.infomat(par = par, dat = 1, method = "exp", nobs = nobs)) %*%
      grad_Nmed
  } else if (type == "mean") {
    if (xi == 0) {
      grad_Nmean <- c(
        1,
        log(N) + emcst,
        0.5 * sigma * (emcst^2 + pi^2 / 6 + 2 * emcst * log(N) + log(N)^2)
      )
    } else {
      grad_Nmean <- c(
        1,
        (N^xi * gamma(-xi + 1) - 1) / xi,
        (N^xi *
          log(N) *
          gamma(-xi + 1) -
          N^xi *
          ...
          sigma /
          xi -
          (N^xi * gamma(-xi + 1) - 1) * sigma / xi^2
      )
    }
    var_Nmean <- t(grad_Nmean) %*%
      solve(gev.infomat(par = par, dat = 1, method = "exp", nobs = nobs)) %*%
      grad_Nmean
  }
  var_est <- switch(
...
gev.abias <- function(shape, rho) {
  ...
  if (shape != 0 && rho < 0) {
    bmu <- (1 + shape) /
      ...
      (-(shape + rho) *
        ...
        shape * (1 - rho) * gamma(1 + shape - rho))
    bsigma <- (-shape -
      rho +
      (1 + shape) * (shape + 2 * rho) * gamma(1 + shape) -
      (1 + shape)^2 * rho * gamma(1 + 2 * shape) +
      shape * gamma(2 - rho) -
      shape * (1 + shape) * (1 - rho) * gamma(1 + shape - rho)) /
      (shape^2 * rho * (shape + rho))
    bshape <- ((shape + rho) *
      (1 + shape + psigamma(1) * shape) -
      (shape + shape^2 * (1 + rho) + 2 * rho * (1 + shape)) *
        gamma(
          1 +
            shape
        ) +
      (1 + shape)^2 * rho * gamma(1 + 2 * shape) +
      shape^2 * gamma(1 - rho) -
      shape * (1 + shape) * gamma(2 - rho) +
      shape * (1 + shape) * (1 - rho) * gamma(1 + shape - rho) -
      shape * rho * psigamma(2 + shape) * gamma(2 + shape) -
      shape^2 *
        psigamma(2 - rho) *
        gamma(2 - rho)) /
      (shape^3 * rho * (shape + rho))
    ...
    bsigma <- (shape^2 *
      gamma(1 + shape) *
      psigamma(1 + shape) -
      (shape + 1)^2 * gamma(1 + 2 * shape) +
      shape^2 * gamma(1 + shape) +
      shape * gamma(1 + shape) * psigamma(1 + shape) -
      (psigamma(1) + 1) * shape +
      3 * shape * gamma(1 + shape) +
      2 *
        gamma(
          1 +
            shape
      ...
      1) /
      shape^3
    bshape <- ((1 + shape + shape * psigamma(1))^2 +
      shape^2 * pi^2 / 6 +
      (1 + shape)^2 * gamma(1 + 2 * shape) -
      2 *
        (1 + shape) *
        ((1 + shape) *
          gamma(1 + shape) +
          shape * psigamma(1 + shape) * gamma(1 + shape))) /
      shape^4
  } else if (rho < 0 && shape == 0) {
      ...
      (-12 *
        ...
        6 *
          ...
          (2 +
            ...
            (-1 + rho) * rho * psigamma(1 - rho)^2 +
            (-1 + rho) * rho * psigamma(1 - rho, deriv = 1)) +
        rho *
          (psigamma(1)^2 *
            (6 - 18 * rho) +
            pi^2 * (1 - 3 * rho) +
            6 * (-psigamma(1)^3) * rho +
            3 * (-psigamma(1)) * (-4 + (4 + pi^2) * rho) +
            6 *
              rho *
              (-2 - 2 * psigamma(1, deriv = 2) + psigamma(2, 2))))
  } else if (rho == 0 && shape == 0) {
  ...
  c(
    solve(gev.infomat(c(0, 1, shape), dat = 1, method = "exp", nobs = 1)) %*%
      c(bmu, bsigma, bshape)
  )
}
...
gpd.abias <- function(shape, rho) {
  stopifnot(rho <= 0, shape > -0.5)
  c(
    -rho / ((1 - rho) * (1 + shape - rho)),
    (1 + shape) / ((1 - rho) * (1 + shape - rho))
  )
}
