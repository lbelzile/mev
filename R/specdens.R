    ...
    if (wgt == "euclidean") {
      if (Rnorm == "l1") {
        return(list(
          ang = ang,
          rad = R,
          wts = as.vector(.EuclideanWeights(
            ang,
            rep(
              1 /
                (ncol(ang) +
      ...
      if (Rnorm == "l1") {
        scel.fit <- .emplik_intern(
          z = ang,
          thresh = 1e-10,
          mu = rep(1 / (ncol(ang) + 1), ncol(ang)),
          lam = rep(0, ncol(ang)),
          eps = 1 / nrow(ang),
          itermax = 10000L
        )
      ...
      if (scel.fit$conv) {
        ...
        warning(
          "Self-concordant empirical likelihood for the mean did not converge."
        )
...
#'  via \code{region=sum}.
#'
#' @param xdat an \code{n} by \code{d} sample matrix
...
#' @param thresh threshold of length 1 for \code{'sum'}, or \code{d} marginal thresholds otherwise.
...
angmeas <- function(
  ...
  if (missing(thresh) & !is.null(args$th)) {
    thresh <- args$th
  }
  if (!is.matrix(xdat)) {
    xdat <- rbind(xdat, deparse.level = 0L)
  }
  if (missing(thresh)) {
    # warning("Threshold set to zero. Using all the data")
    thresh <- 0
  } else if (any(thresh >= 1, thresh < 0)) {
  ...
  marg <- match.arg(
    tolower(marg),
    choices = c("frechet", "pareto"),
    several.ok = FALSE
  )
  wgt <- match.arg(
    tolower(wgt),
    choices = c("empirical", "euclidean"),
    several.ok = FALSE
  )
  region <- match.arg(region)
  if (!is.angle) {
    # Use only complete cases x <- na.omit(x)
    # Margins are transformed to unit Frechet/Pareto (PIT)
    S <- switch(
      marg,
      frechet = -1 /
        log(na.omit(
          apply(xdat, 2, rank, na.last = "keep", ties.method = "random") /
            (nrow(xdat) +
              1)
        )),
      pareto = 1 /
    ...
    R <- switch(
      Rnorm,
      l1 = rowSums(S),
      l2 = apply(S, 1, function(x) {
        sqrt(sum(x^2))
      }),
      linf = apply(S, 1, max)
    )
    # Ordering observations: this is not strictly necessary
    # but of some use for later inspection + for
    # defn of 'above' when region='sum'
    ...
    if (region != "sum") {
      if (ncol(S) != length(thresh)) {
        if (length(thresh) == 1) {
          warning(
            "The length of the threshold provided does not match the dimension of the problem; assuming identical thresholds"
          )
          thresh <- rep(thresh[1], ncol(S))
        } else {
          stop(
            "Invalid argument \"thresh\": the length of the threshold provided does not match the dimension of the problem."
          )
        }
      }
      if (wgt == "euclidean") {
      ...
      if (region == "min") {
        above <- Reduce(
          ...
          sapply(
            1:ncol(S),
            function(i) {
              which(S[, i] > quantile(S[, i], thresh[i]))
            },
            simplify = FALSE
          )
        )
      ...
      if (length(thresh) != 1) {
        warning("Only using the first entry of \"thresh\"")
        thresh <- thresh[1]
      }
      above <- floor(thresh * length(R)):length(R) #works because observations are ordered
    }
    ...
    if (Rnorm == Anorm) {
      ...
      if (ncol(S) != 2) {
        stop(
          "Invalid norm for sample, arctan transformation only for bivariate samples"
        )
      }
      ang <- atan(S[, 2] / S[, 1])
    } else {
...
.pickands.emp <- function(
  ...
  if (is.null(emp$wts)) {
    warning(
      "Invalid input; no \"wts arguments, likely due to convergence failure"
    )
...
angmeasdir <- function(
  ...
  for (i in 1:n) {
    loowts[i, -i] <- .returnAng(
      ang = angmeasure$ang[-i, , drop = FALSE],
      R = NULL,
      Rnorm = Rnorm,
      wgt = wgt,
      region = region
  ...
  return(invisible(list(
    nu = optnu$minimum,
    dirparmat = optnu$minimum * angles,
    wts = angmeasure$wts
  )))
}
