.auglag <- 
  function (x0, fn, gr = NULL, lower = NULL, upper = NULL, hin = NULL, 
...
#' @title Generalized Pareto maximum likelihood estimates for various quantities of interest
#' 
...
#' @title Generalized extreme value maximum likelihood estimates for various quantities of interest
#' 
...
#' Spline correction for rstar
#'
#' The statistic \code{rstar} obtained from fitting a tangent exponential model 
#' (or more generally derived from an approximation of \eqn{p^*} from Barndorff-Nielsen)
#' can be numerically unstable for values close to \eqn{r=0}.
#' This function `corrects` these incorrect values deemed outlying and replaces them with interpolated values using splines.
...
#' may also be driven by unusual outlying patterns or fail at detecting outliers 
#' (or falsely identifying `correct' values as outliers). The user should always validate by 
#' comparing the plots of both the uncorrected (raw) output of the \code{\link{gpd.tem}} or
#'  \code{\link{gev.tem}} with that of \code{spline.corr}.
...
gev.pll <- function(psi, param = c("loc", "scale", "shape", "quant", "Nmean", "Nquant"),  
                    mod = c("tem","modif"), dat, N, p, q, correction = TRUE, ...){
  
  oldpar <- param <- match.arg(param, c("loc", "scale", "shape", "quant", "Nmean", "Nquant"))
  ...
  if(param %in% c("loc", "scale", "shape")){
    ...
    constr.mle.scale <- function(sigmat, dat = dat){
      opt <- nloptr::sbplx(control = list(print_level=1), x0 = c(median(dat/sigmat), 0.05), fn = function(par){-gev.ll(c(par[1], sigmat, par[2]), dat = dat)})
    ...
    if("tem" %in% mod){
      ...
      tem.max.opt <- function(psi){
       ...
       tem.max <- optim(par = mle[ind], fn = tem.max.opt, method = "Brent", 
    ...
    if("tem" %in% mod){
      ...
      tem.max <- optim(par = mle[1], fn = tem.max.opt, method = "Brent", 
                       lower = max(1e-5, mle[1] - std.error), 
...
gpd.pll <- function(psi, param = c("scale", "shape", "quant", "VaR", "ES", "Nmean", "Nquant"), 
                    mod = c("tem","modif"), mle = NULL, dat, m, N, p, q, correction = TRUE, ...){
  ...
  if(param == "scale"){
    ...
    if("tem" %in% mod){
      ...
      tem.max <- optim(par=mle[2], fn=tem.max.opt, method = "Brent", 
