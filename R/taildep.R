...
taildep <- function(
  ...
  if (is.character(depmeas)) {
    depmeas <- which(
      c("eta", "chi") %in%
        match.arg(depmeas, c("eta", "chi"), several.ok = TRUE)
    )
  }
  ...
  if (empirical.transformation) {
    ...
    if ("betacop" %in% method) {
      if (ties.method != "random") {
        warning(
          "Beta copula does not allow for ties; switching to `ties.method = \"random\"`"
        )
        ties.method <- "random"
      }
    }
    ...
    if ("betacop" %in% method) {
      warning(
        "Beta copula is rank-based; ignoring argument \"empirical.transformation\"."
      )
      datarank <- apply(data, 2, rank, ties.method = ties.method)
    }
    # observations are already uniform
    rowmin <- apply(data, 1, min)
    stopifnot(min(rowmin) > 0, max(rowmin) < 1)
  }
  ...
  if ("emp" %in% method) {
    ...
    if (1 %in% depmeas && methodeta == "emp") {
      ...
      seeta <- sqrt(
        (((log(1 - u)^2) / (log(cbaru)^4 * cbaru)) * (1 - cbaru)) / n
      )
      est_eta <- cbind(
        eta = etau,
        etalow = etau - cnst * seeta,
        etaupp = etau + cnst * seeta
      )
    }
    if (2 %in% depmeas && methodchi == "emp") {
      ...
      est_chi <- cbind(
        chi = chiu,
        chilow = chiu - cnst * sechi,
        chiupp = chiu + cnst * sechi
      )
    }
  }
  if (1 %in% depmeas && methodeta == "gpd") {
    ...
    if (confint == "lrt") {
      est_eta <- t(sapply(1 / (1 - u), function(th) {
        ...
        if (sum(ps > th) > 15) {
          ...
          prof <- try(suppressWarnings(gpd.pll(
            param = "shape",
            dat = ps,
            threshold = th,
            mod = "prof",
          ...
          co <- try(suppressWarnings(confint(
            prof,
            prob = c((1 - level) / 2, (1 + level) / 2),
            print = FALSE
          )))
        ...
        if (sum(ps > th) > 15) {
          ...
          if (fitu$estimate[2] < -0.5) {
            ...
            c(
              fitu$estimate[2],
              fitu$estimate[2] +
                c(-1, 1) * fitu$std.err[2] * qnorm((1 + level) / 2)
            )
          }
        } else {
          return(rep(NA, 3))
    ...
    est_eta <- t(sapply(-log(1 - u), function(th) {
      ...
      if (confint == "lrt") {
        low <- try(uniroot(
          f = function(il) {
            2 * (expll(samp, il) - maxll) + qchisq(level, 1)
          },
          interval = c(1e-8, mle)
        ))
        upp <- try(uniroot(
        ...
        return(c(
          mle,
          mle + c(-1, 1) * mle / sqrt(length(samp)) * qnorm((1 + level) / 2)
        ))
      }
    }))
  }
  ...
  if (sum(depmeas) == 0) {
    ...
    if (!is.null(ellips$which)) {
      if (is.character(ellips$which)) {
        show <- c("eta", "chi") %in%
          match.arg(ellips$which, choices = c("eta", "chi"), several.ok = TRUE)
...
kjtail <- function(
  ...
  neglik_fn <- function(par, xdat, Tp) {
    ...
    return(
      -sum(log(pmax(
        1e-20,
        k1 +
          (1 - k1 * Tp) / eta * exp((1 / eta - 1) * log(xdat) - log(Tp) / eta)
      )))
    )
  }
  for (j in seq_along(qlev)) {
    ...
    est <- Rsolnp::solnp(
      ...
      ineqfun = function(par, xdat, Tp) {
        ...
        range(
          par[1] *
            xdat +
            (1 - par[1] * Tp) * exp((log(xdat) - log(Tp)) / par[2])
        )
      },
      ineqLB = rep(0, 2),
      ineqUB = rep(1, 2),
    ...
    if (isTRUE(est$convergence == 0)) {
      ...
      etasd_j <- try(
        silent = TRUE,
        suppressWarnings(sqrt(solve(est$hessian)[2, 2]))
      )
  ...
  if (!is.null(ptail)) {
    if (isTRUE(all(length(ptail) == 1L, ptail > 0, ptail < min(plev)))) {
      ret_list$tailprob <- as.numeric(
        nabove /
          n *
          (k1hat * ptail + (1 - k1hat * plev) * (ptail / plev)^(1 / etahat))
      )
    } else {
      warning(
        "Invalid \"ptail\" argument. Must be a scalar smaller than the probability selected."
      )
    }
  }
